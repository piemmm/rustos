...
18. **Check the `plans/` jump-sheet before touching a covered area.** Every
    ...
    | Sound: the one audio path (client stream ABI, the `audiod` mixer/router, the `audiochan-v1` device channel), the exported device clock and latency grant, seat-leased sinks and gated capture, the file decoders (`lib/sound`), their sandboxed decode and the one feature-gated encoder (FLAC), the audio driver class, the two players (`play`, `music.app`), and the default desktop sounds (the `SoundEvent` vocabulary, the shipped FLAC masters, the `soundd` cue authority and who may cue what) | `plans/SOUND.md` |
    ...
    | Service manager: service lifecycle/readiness, discovery vs registration vs on-demand endpoint activation, system- vs user-scope managers, idle linger, stop/shutdown ordering, restart policy, the control endpoint and `servicectl` | `plans/NEW-SERVICEMANAGER.md` |
    ...
    | Boot namespace assembly: which volume backs which root, the signed namespace policy on `/System`, volume attachment vs view projection, mount-flag narrowing, and the user-space root-volume unlock | `plans/NEW-NAMESPACE.md` |
    | Advisory file locking: byte-range locks owned by the open file description, the conflict/fairness/deadlock rules, the `file-locks` bound, `fs_lock`/`fs_lock_query`, and the `flock` command | `plans/FILELOCK.md` |
    ...
    | Link-local service discovery: the mDNS/DNS-SD engine, the three-gate publication authority (attested identity, granted type, demonstrated port ownership), the unprivileged socket-owning decoder, per-interface posture and cache scoping, and the permanent LLMNR/NetBIOS exclusion | `plans/ZEROCONF.md` |

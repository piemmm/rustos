...
#ifndef TAIRIX_STDINFO_H
#define TAIRIX_STDINFO_H

#include <stdint.h>

/* Reserved stdinfo file descriptor; no component may repurpose it. */
#define TAIRIX_STDINFO_FD 3u
/* stdinfo framing version tag for the frozen v1 framing. */
#define TAIRIX_STDINFO_VERSION_V1 1u
/* stdinfo framing version this header set describes. */
#define TAIRIX_STDINFO_VERSION_CURRENT 1u

/* Closed set of record kinds (uint8_t). Wire spelling is the string in parens. */
#define TAIRIX_STDINFO_KIND_OMISSION ((uint8_t)0u) /* "omission" */
#define TAIRIX_STDINFO_KIND_SUMMARY ((uint8_t)1u) /* "summary" */
#define TAIRIX_STDINFO_KIND_SCHEMA ((uint8_t)2u) /* "schema" */
#define TAIRIX_STDINFO_KIND_SUGGESTION ((uint8_t)3u) /* "suggestion" */
#define TAIRIX_STDINFO_KIND_CONTEXT ((uint8_t)4u) /* "context" */

/* Advisory severity (uint8_t). Security events use lib/log, not fd 3. */
#define TAIRIX_STDINFO_SEVERITY_INFO ((uint8_t)0u) /* "info" */
#define TAIRIX_STDINFO_SEVERITY_DEBUG ((uint8_t)1u) /* "debug" */

#endif /* TAIRIX_STDINFO_H */

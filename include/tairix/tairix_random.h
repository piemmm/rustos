...
#ifndef TAIRIX_RANDOM_H
#define TAIRIX_RANDOM_H

#include <stdint.h>

/* Request flags (uint32_t). Every undefined bit is reserved and must be zero. */
#define TAIRIX_RANDOM_FLAG_NON_BLOCKING 0x1u

/* Default per-CPU random output reserve, in bytes. */
#define TAIRIX_RANDOM_RESERVE_DEFAULT_BYTES ((uintptr_t)2048u)
/* Maximum number of bytes a single random request may ask for. */
#define TAIRIX_RANDOM_REQUEST_MAX_BYTES ((uintptr_t)65536u)

#endif /* TAIRIX_RANDOM_H */

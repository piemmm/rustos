 ...
 *     uint8_t key_len   (<= ROS_LOG_FIELD_KEY_MAX)
 *     key bytes         (key_len, UTF-8)
 *     a typed field value: a 1-byte ROS_FIELD_TAG_* discriminant
 *       followed by its little-endian payload. The whole encoded
 *       value is <= ROS_LOG_FIELD_VALUE_MAX bytes. Payloads:
...
#define ROS_LOG_FIELD_VALUE_MAX ((uintptr_t)256u)
/* Fixed record header length, in bytes. */
#define ROS_LOG_RECORD_HEADER_LEN ((uintptr_t)8u)
/* Maximum encoded record length, in bytes. */
#define ROS_LOG_RECORD_MAX ((uintptr_t)2440u)

/* Field-value type tags: the first byte of an encoded field value. */
#define ROS_FIELD_TAG_NULL ((uint8_t)0u)
#define ROS_FIELD_TAG_BOOL ((uint8_t)1u)
#define ROS_FIELD_TAG_SIGNED ((uint8_t)2u)
#define ROS_FIELD_TAG_UNSIGNED ((uint8_t)3u)
#define ROS_FIELD_TAG_DECIMAL ((uint8_t)4u)
#define ROS_FIELD_TAG_TIME ((uint8_t)5u)
#define ROS_FIELD_TAG_DURATION ((uint8_t)6u)
#define ROS_FIELD_TAG_STR ((uint8_t)7u)
#define ROS_FIELD_TAG_BYTES ((uint8_t)8u)
#define ROS_FIELD_TAG_UUID ((uint8_t)9u)
#define ROS_FIELD_TAG_IP ((uint8_t)10u)
#define ROS_FIELD_TAG_MAC ((uint8_t)11u)
#define ROS_FIELD_TAG_ERROR ((uint8_t)12u)
#define ROS_FIELD_TAG_CAP ((uint8_t)13u)
#define ROS_FIELD_TAG_LIST ((uint8_t)14u)

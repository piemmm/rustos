...
use rustos_kernel_mem::{AddressSpace, DirectPhysMap, UserStack};
...
impl InitSpawn for Aarch64InitSpawn {
    fn spawn_init(&self, ctx: &dyn InitSpawnCtx) {
        ...
        // Register PID 1's caps, publish it as the current task, and
        // dispatch it — `admit_init` diverges into EL0 on success.
        // SAFETY: the image was built into and `space` switched to active
        // above, and the trap path is installed, per the
        // `InitSpawnCtx::admit_init` contract.
        unsafe {
            ctx.admit_init(init_caps(), enter);
        }
    }
}

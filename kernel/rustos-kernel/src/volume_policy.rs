...
//!   [`rustos_users::STORAGE_GROUP`] group's gid into, resolved **by
...
//!   `set_security` stays refused (the format cannot store a RustOS
...
impl<F: FilesystemRead> FilesystemSecurity for GroupMappedFs<F> {
    ...
    fn set_security(&mut self, _node: NodeId, _security: NodeSecurity) -> Result<(), DriverError> {
        ...
        // underlying format cannot hold a RustOS security record; storing

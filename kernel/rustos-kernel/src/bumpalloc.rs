//! Forward-only bump allocator for the boot heap.
//!
...
//! It satisfies `AGENTS.md` §15.1 — *do not be lazy* — for the (c7-bin)
...
//!   `#[global_allocator]`-bearing crates together; the production bin
//!   declares one and the QEMU integration test bin declares its own.
//!
...
/// `BTreeSet` free-lists in the buddy allocator, the
/// `Scheduler<BinArch>` per-CPU bookkeeping for the BSP, the
/// `Arc<BinArch>`, and the audit-event Sink formatting allocations
/// emitted during the `mem` / `sec` / `sched` / `ipc` phases.
/// Production per-process heaps land in a later stage; this value is
/// the documented temporary ceiling.
...
/// (`#[repr(C, align(4096))]`) in the bin crate, then assign the
/// `cursor` field to a freshly-constructed `AtomicUsize::new(0)` and
/// register it via `#[global_allocator]`.
///

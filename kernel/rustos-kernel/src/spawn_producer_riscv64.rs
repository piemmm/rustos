...
/// vector (`AGENTS.md` §5.2, §16.5): the login session supervisor
/// (`plans/PI.md` P11) and the `Shell` session program (`plans/SPAWN.md`
/// `SP3b`). The `rxe`s are baked at build time (`build.rs`), exactly like
/// the aarch64 producer's registry (`AGENTS.md` §2.2). Login additionally
/// holds `PROC_SPAWN` (it launches the shell) and `USERS_READ` (it
/// verifies credentials against the kernel-held user database); the shell
/// holds only the console pair.

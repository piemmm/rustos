...
use rustos_arch_x86_64::kernel_arch::halt as arch_halt;
...
///
/// Always returns `!`. Forwards to `rustos_kernel_core::handle_panic`
...
pub fn handle_panic_via_kernel_core(info: &PanicInfo<'_>) -> ! {
    ...
    if raw.is_null() {
        ...
        let _ = writeln!(s, "[rustos-kernel] panic before init: {info}");
        arch_halt()
    } else {

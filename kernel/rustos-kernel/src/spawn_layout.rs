...
pub static SPAWN_PROGRAMS: [EmbeddedProgram; 7] = [
    EmbeddedProgram {
        path: SHELL_PATH,
        rxe: SHELL_RXE,
    ...
    // The System Information service (`sysinfod`, `AGENTS.md` §16.6): the sole
    // holder of `CAP_SYSINFO_INTROSPECT` (the privileged, unfiltered global
    // introspection primitive) and the trusted scoping broker for every
    // `sysinfo` client. It binds the well-known `SYSINFO_ENDPOINT`, reads the
    // kernel's global process/memory/mount/identity/uptime/limits view through
    // `sysinfo_introspect`, and re-derives each client's scope against that
    // client's kernel-attested `Origin` before replying. It requests
    // `CAP_SYSINFO_HW` for the hardware-tree query (`hw_tree_read`) and
    // `CAP_LOG_EMIT` for its structured audit records — nothing more (least
    // privilege). It binds an *unrestricted-sender* endpoint (per-query scoping
    // is enforced in the broker, not the transport), so it needs no
    ...
    // The `ps` tool: a shell spawns it to list processes over the `sysinfo`
    // API. It writes its listing to standard output (fd 1) and diagnostics to
    // standard error (fd 2), so it holds `CAP_CONSOLE_WRITE` and nothing more:
    // the `sysinfo` endpoint is unrestricted-sender (no capability to call),
    // and every per-query scope is enforced by `sysinfod` against this
    // process's kernel-attested origin — a global listing it is not entitled
    // to comes back as a fail-closed `PermissionDenied`, never widened here.
    // Its baked default argument lists the caller's own processes.
    EmbeddedProgram {
        path: PS_PATH,
        rxe: PS_RXE,
        caps: &[CapabilityId::CONSOLE_WRITE],
        args: &[b"ps"],
    },
    // The `sysinfo` tool: the umbrella terminal client of the System
    // Information API. Like `ps` it writes fd 1 / fd 2 and calls the
    // unrestricted `sysinfo` endpoint, so it holds only `CAP_CONSOLE_WRITE`;
    // per-query authority stays with `sysinfod` and the caller's attested
    // origin. Its baked default argument prints the usage banner.
    EmbeddedProgram {
        path: SYSINFO_PATH,
        rxe: SYSINFO_RXE,
        caps: &[CapabilityId::CONSOLE_WRITE],
    ...
    // The `top` tool: a live process-overview TUI over the `sysinfo` API. It
    // draws a full-screen display to standard output (fd 1) and reads
    // keystrokes from standard input (fd 0) in raw mode, so it holds
    // `CAP_CONSOLE_WRITE` *and* `CAP_CONSOLE_READ` (the latter also authorises
    // the `set_echo` it uses to suppress local echo) — nothing more. Like `ps`
    // it calls the unrestricted `sysinfo` endpoint, and every per-query scope
    // (its `a` all-processes toggle) is enforced by `sysinfod` against this
    // process's kernel-attested origin, never widened here.
    EmbeddedProgram {
        path: TOP_PATH,
        rxe: TOP_RXE,
        caps: &[CapabilityId::CONSOLE_WRITE, CapabilityId::CONSOLE_READ],

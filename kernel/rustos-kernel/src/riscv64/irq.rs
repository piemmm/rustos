...
//! N4e-riscv64).
//!
//! The production boot path runs [`rustos_arch_riscv64::trap::install_trap_vector`]
...
//! [`rustos_arch_riscv64::trap::set_supervisor_interrupts`] so no interrupt
...
/// access), and hands it to its [`rustos_kernel_core::IrqParkWaiter`] as the
/// re-arm controller.
#[must_use]
...
extern "C" fn production_external_dispatch() {
    ...
    if source != 0 {
        ...
        rustos_kernel_core::note_preempt_tick(rustos_arch_riscv64::smp::current_hartid());
    }
}

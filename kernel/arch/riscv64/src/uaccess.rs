...
//! `[tairix_riscv64_uaccess_window_begin, tairix_riscv64_uaccess_window_end)`,
...
//! rewrites the frame's `sepc` to `tairix_riscv64_uaccess_fixup` — the
...
.section .text
.balign 4
.global tairix_riscv64_guarded_user_copy
.global tairix_riscv64_uaccess_window_begin
.global tairix_riscv64_uaccess_window_end
.global tairix_riscv64_uaccess_fixup
# usize tairix_riscv64_guarded_user_copy(u8 *dst, const u8 *src, usize len)
...
tairix_riscv64_guarded_user_copy:
    ...
    mv      t1, a1
    mv      t2, a2
tairix_riscv64_uaccess_window_begin:
...
4:  # byte tail (and the mutually-misaligned body)
    ...
    j       4b
5:
tairix_riscv64_uaccess_window_end:
    li      a0, 0
    ret
.balign 4
tairix_riscv64_uaccess_fixup:
    li      a0, 1
    ret
"#
);
...
extern "C" {
    ...
    fn tairix_riscv64_guarded_user_copy(dst: *mut u8, src: *const u8, len: usize) -> usize;
    /// First instruction of the fault window (inclusive).
    fn tairix_riscv64_uaccess_window_begin();
    /// First instruction past the fault window (exclusive).
    fn tairix_riscv64_uaccess_window_end();
    /// Recovery entry: returns `1` from the interrupted copy.
    fn tairix_riscv64_uaccess_fixup();
}
...
pub fn kernel_fixup_for(pc: u64) -> Option<u64> {
    let begin = tairix_riscv64_uaccess_window_begin as *const () as usize as u64;
    let end = tairix_riscv64_uaccess_window_end as *const () as usize as u64;
    if tairix_arch_api::uaccess::pc_in_window(pc, begin, end) {
        Some(tairix_riscv64_uaccess_fixup as *const () as usize as u64)
    } else {
        None
    }
}

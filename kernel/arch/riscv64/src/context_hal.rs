...
//! reinterprets the neutral [`TaskContext`](tairix_arch_api::TaskContext)
...
impl ContextSwitchHal {
    /// Construct the riscv64 context-switch handle.
    #[must_use]
...
impl ContextSwitch for ContextSwitchHal {
    ...
    unsafe fn switch(&self, prev: *mut TaskContext, next: *mut TaskContext) {
        ...
        {
            let _ = (prev, next);
            unreachable!("context switch is only meaningful on the riscv64 bare-metal target")
        }
    }
}

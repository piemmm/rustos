// RustOS wasm32 host loader (`AGENTS.md` §3 / Stage 3d, multi-worker
...
//   rustos_host_start_worker(w)  -> spawn a Web Worker as logical CPU w
...
// calls `rustos_host_start_worker(n)`, the main thread spawns a real Web
...
// (`rustos_host_post_ipi`) is a post on that channel that re-enters the
// target's `rustos_arch_wasm32_on_message`. The main thread is the hub:
...
function makeEnv(ctx) {
  ...
  return {
    rustos_host_now_ms: () => ctx.now(),
    rustos_host_current_worker: () => ctx.worker >>> 0,
    rustos_host_post_ipi: (w) => ctx.postIpi(w >>> 0),
    rustos_host_start_worker: (w) => (ctx.startWorker(w >>> 0) ? 1 : 0),
    rustos_host_request_frame: () => {
      ctx.requestFrame(() =>
        ctx.getInstance().exports.rustos_arch_wasm32_on_frame(),
      );
    },
...
/**
 * Instantiate a RustOS wasm32 module against a host context.
 *
 ...
 * then calls `rustos_arch_wasm32_main()`.
 *
...
/**
 * Instantiate and boot a RustOS wasm32 module on the main thread as
...
export async function boot(wasmBytes, hooks = {}) {
  ...
  const deliverIpi = (target) => {
    if (target === 0) {
      queueMicrotask(() =>
        ctx.getInstance().exports.rustos_arch_wasm32_on_message(),
      );
      return;
    }
 ...
 * to `rustos_arch_wasm32_on_message`, and runs the boot trampoline.
 */
  ...
  globalThis.onmessage = async (event) => {
    ...
    port.onmessage = (ev) => {
      if (ev.data && ev.data.t === "ipi")
        instance.exports.rustos_arch_wasm32_on_message();
    };
    port.start?.();
    instance.exports.rustos_arch_wasm32_main();
  };

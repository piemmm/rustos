...
//! `web/tairix.js` supplies them.
//!
//! TAIRiX deliberately does **not** depend on `wasm-bindgen` / `web-sys`
...
// module name the companion glue (`web/tairix.js`) supplies them under.
...
extern "C" {
    ...
    fn tairix_host_current_worker() -> u32;
    ...
    fn tairix_host_post_ipi(worker: u32);
    ...
    fn tairix_host_start_worker(worker: u32) -> u32;
    ...
    /// surface returns `0`.
    fn tairix_host_present_framebuffer(
        ptr: *const u8,
        len: usize,
        width: u32,
        height: u32,
...
pub fn host_console_write(bytes: &[u8]) {
    ...
    unsafe { tairix_host_console_write(bytes.as_ptr(), bytes.len()) }
}
...
pub fn host_logical_processors() -> u32 {
    ...
    unsafe { tairix_host_logical_processors() }
}
...
pub fn host_start_worker(worker: u32) -> bool {
    // SAFETY: `tairix_host_start_worker` takes a plain integer and has no
    ...
    unsafe { tairix_host_start_worker(worker) != 0 }
}
...
pub fn host_has_display() -> bool {
    ...
    unsafe { tairix_host_has_display() != 0 }
}
...
pub fn host_present_framebuffer(frame: &[u8], width: u32, height: u32, stride: u32) -> u32 {
    ...
    unsafe { tairix_host_present_framebuffer(frame.as_ptr(), frame.len(), width, height, stride) }
}

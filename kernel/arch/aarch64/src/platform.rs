...
//! windows, `plans/PI.md` P3), and the console UART (P2). Richer
//! enumeration (CPU nodes from `/cpus`, virtio-mmio children) lands behind
//! this same trait as the reader grows.
//!
...
mod tests {
    ...
    fn emits_serial_node_with_discovered_base_from_raspi_tree() {
        // The Pi-shaped fixture carries a GIC-400, a PL011 + a mini-UART
        // and no `/timer`; discovery emits root + memory + the GIC + the
        // (preferred) PL011 serial node, each with its MMIO base read from
        // the tree.
        ...
        let mut sink = CountingSink::default();
        disco.discover(&mut sink).expect("discovery succeeds");
        assert_eq!(sink.total, 4, "root + memory + gic + serial");
        assert_eq!(sink.memory, 1);

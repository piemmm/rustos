//! aarch64 task context primitive.
//!
//! The aarch64 analogue of `kernel/arch/{x86_64,riscv64}::context`. It
//! defines [`TaskCtx`] — the per-task register-save area the scheduler
//! parks in its task table — and `switch`, the bare-metal switch
//! primitive. The contract every architecture port owes `kernel/sched`
//! is identical: a stable `*mut TaskCtx` layout plus an `extern "C"`
//! switch invoked at a preemption / yield point.
//!
//! # Layout
//!
...
//!   this CPU for the call, and 16-byte aligned.
...
impl TaskCtx {
    ...
    /// Seed an initial frame so the first `switch` *into* this task lands
    /// at `entry` with the first argument register `x0` set to `arg`.
    ///
    ...
    /// registers, loads `x0 = arg`, and `ret`s into `entry`.
    ///
    /// # Errors
    ///
    /// [`PrepareError::Misaligned`] if the region's top is not 16-byte
    /// aligned (AAPCS64 stack alignment); [`PrepareError::TooSmall`] if
    /// it has no room for the synthesised frame.
    pub fn prepare(
        &mut self,
        ...
        // in ascending address order from `sp` (8-byte slots):
        //
        //   [sp + 0x00] x19   (callee-saved, seeded to 0)
        //   ...               (x20..x28, seeded to 0)
        ...
        unsafe {
            ...
            for i in 0..FRAME_BYTES / size_of::<u64>() {
                p.add(i).write(0);
            }
            // x30 <- entry (index 11, offset 0x58)
            p.add(11).write(entry as usize as u64);
            // x0 <- arg (index 12, offset 0x60)
            p.add(12).write(arg as u64);
            ...
            p.add(22).write(INITIAL_DAIF_INHERIT);
        }
...
/// running task and is exclusive to this CPU, `next.sp` is zero
/// (unreachable) or a value `switch`/[`TaskCtx::prepare`] wrote, and the
/// inbound kernel stack is mapped and exclusive to this CPU.
///

...
impl<A: SchedulerArch> Scheduler<A> {
    ...
    pub fn unpark(&self, id: TaskId) -> SchedResult<()> {
        ...
        match task.load_state() {
            TaskState::Exited => return Err(SchedError::InvalidState),
            // Not committed to park yet (running its body, or already
            // queued): a CAS to Ready would be a no-op, so record a
            // pending token instead — the dispatch loop's `Park` commit
            // consumes it and re-readies the task (no lost wake-ups).
    ...
    fn dispatch(&self, cpu: CpuId, id: TaskId) -> StepOutcome {
        ...
        // Close the park/unpark lost-wakeup race: a token left by an
        // `unpark` that arrived mid-run cancels the park and re-readies
        // the task. Consumed exactly when we would have parked.

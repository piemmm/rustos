...
use crate::program_manifests::{
    AUDIOD_MANIFEST, CAT_MANIFEST, CLEAR_MANIFEST, CONFD_MANIFEST, DEVMGR_MANIFEST,
    DISCOVERYD_MANIFEST, FONTD_MANIFEST, LOGIN_MANIFEST, LS_MANIFEST, MAN_MANIFEST,
    NETSTACK_MANIFEST, PS_MANIFEST, RESET_MANIFEST, SEATMGR_MANIFEST, SHELL_MANIFEST,
    STRESS_MANIFEST, SYSINFOD_MANIFEST, SYSINFO_MANIFEST, SYSMON_MANIFEST, TIMED_MANIFEST,
    TOP_MANIFEST, USERS_TOOL_MANIFEST,
};
...
mod program_rxes {
    ...
    // `FONTD_RXE: &[u8]` and `FONTD_USER_BIAS: u64`, generated by `build.rs`.
    include!(concat!(env!("OUT_DIR"), "/fontd_rxe.rs"));
    // `CONFD_RXE: &[u8]` and `CONFD_USER_BIAS: u64`, generated by `build.rs`.
    include!(concat!(env!("OUT_DIR"), "/confd_rxe.rs"));
    // `AUDIOD_RXE: &[u8]` and `AUDIOD_USER_BIAS: u64`, generated by `build.rs`.
    include!(concat!(env!("OUT_DIR"), "/audiod_rxe.rs"));
    // `TIMED_RXE: &[u8]` and `TIMED_USER_BIAS: u64`, generated by `build.rs`.
    include!(concat!(env!("OUT_DIR"), "/timed_rxe.rs"));
    // `DISCOVERYD_RXE: &[u8]` and `DISCOVERYD_USER_BIAS: u64`, generated by `build.rs`.
    include!(concat!(env!("OUT_DIR"), "/discoveryd_rxe.rs"));
    ...
    const _: () = assert!(FONTD_USER_BIAS == super::CHILD_USER_BIAS);
    const _: () = assert!(CONFD_USER_BIAS == super::CHILD_USER_BIAS);
    const _: () = assert!(AUDIOD_USER_BIAS == super::CHILD_USER_BIAS);
    const _: () = assert!(TIMED_USER_BIAS == super::CHILD_USER_BIAS);
    const _: () = assert!(DISCOVERYD_USER_BIAS == super::CHILD_USER_BIAS);
...
#[cfg(not(all(freestanding, kernel_isa = "aarch64")))]
use program_rxes::{
    AUDIOD_RXE, CAT_RXE, CLEAR_RXE, CONFD_RXE, DEVMGR_RXE, DISCOVERYD_RXE, FONTD_RXE, LOGIN_RXE,
    LS_RXE, MAN_RXE, NETSTACK_RXE, PS_RXE, RESET_RXE, SEATMGR_RXE, SHELL_RXE, STRESS_RXE,
    SYSINFOD_RXE, SYSINFO_RXE, SYSMON_RXE, TIMED_RXE, TOP_RXE, USERS_CLI_RXE,
};
...
pub use crate::spawn_paths::{
    ...
    LOGIN_PATH, LS_PATH, MAN_PATH, NETSTACK_PATH, PS_PATH, RESET_PATH, SEATMGR_PATH, SHELL_PATH,
    STRESS_PATH, SYSINFOD_PATH, SYSINFO_PATH, SYSMON_PATH, TIMED_PATH, TOP_PATH, USERS_CLI_PATH,
};
...
#[cfg(not(all(freestanding, kernel_isa = "aarch64")))]
pub static SPAWN_PROGRAMS: &[EmbeddedProgram] = &[
    EmbeddedProgram {
        path: SHELL_PATH,
        rxe: SHELL_RXE,
        caps: SHELL_MANIFEST,
    ...
    EmbeddedProgram {
        path: DISCOVERYD_PATH,
        rxe: DISCOVERYD_RXE,
        caps: DISCOVERYD_MANIFEST,
        args: &[b"discoveryd"],
    },
    EmbeddedProgram {
        path: PS_PATH,

...
//! [`tairix_abi::SyscallNumber::IPC_CALL`] surface served by a
//! [`tairix_kernel_ipc::CallEndpoint`] this server drains.
//!
...
//! decodes one [`tairix_abi::driver_store::StoreRequest`] and serves it
...
//! [`tairix_abi::driver_store`] wire encoders. [`serve_pending`] glues that
...
use tairix_abi::driver_store::{self, StoreRequest, DRIVER_STORE_ENDPOINT, MAX_REQUEST_LEN};
...
/// [`tairix_abi::CapabilityId::DRV_LOAD`] (the device manager's authority to
/// read the store); binding such a restricted-sender
/// endpoint requires the `creator` to hold
/// [`tairix_abi::CapabilityId::IPC_BIND_PRIVILEGED`]. The server (the
...
pub fn create_driver_store_endpoint<S: Sink + ?Sized>(
    ...
    required_send.insert(tairix_abi::CapabilityId::DRV_LOAD);
    CallEndpoint::create(
...
fn catalogue_reply(buf: &mut [u8], store: &DriverStore) -> Result<usize, Errno> {
    ...
    let mut entries: Vec<(u32, &[tairix_abi::DriverBindKey])> = Vec::with_capacity(drivers.len());
    ...
    F: FilesystemRead + FilesystemSecurity + ?Sized,
{
    ...
    match endpoint.reply(call.ticket, &reply, audit) {
        Ok(0) => tairix_kernel_core::call_wake(),
        Ok(poster) => tairix_kernel_core::call_wake_task(poster),
        Err(_) => {}
    }
    true
}
...
/// [`tairix_abi::CapabilityId::IPC_BIND_PRIVILEGED`] — see
...
pub fn serve_system_store(
    ...
    tairix_kernel_core::callreg::register(endpoint.clone(), audit)
    ...
    loop {
        ...
        match crate::unlock_service::store_service_task() {
            Some(task) => {
                tairix_kernel_core::SERVE_WAITQ.register(task, tairix_kernel_core::NO_DEADLINE);
                // Re-check after registering: if a call arrived in the
                // register window, serve it and skip the park.
                if serve_pending(&service, ctx, &store, &endpoint, audit) {
                    tairix_kernel_core::SERVE_WAITQ.deregister(task);
                    continue;
                }
                coop.park();
                tairix_kernel_core::SERVE_WAITQ.deregister(task);
...
mod tests {
    ...
    impl tairix_kernel_core::YieldHandle for UnreachableYielder {
    ...
    fn serve_system_store_without_bind_authority_fails_closed_and_registers_nothing() {
        ...
        assert!(
            !tairix_kernel_core::callreg::contains(EndpointId(DRIVER_STORE_ENDPOINT)),

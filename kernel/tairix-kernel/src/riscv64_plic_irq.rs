...
//! [`tairix_arch_riscv64::plic::PlicController`] exposes an inherent
//! `arm`/`mask`/`unmask`/`claim`/`complete` surface (and implements the Arch
//! HAL's [`tairix_arch_api::IrqController`]), but it does not implement the
//! architecture-neutral [`tairix_kernel_irq::IrqController`] trait
//! [`tairix_kernel_irq::IrqTable::fire`] and [`IrqParkWaiter`] drive. Rust's
...
//!
//! [`IrqParkWaiter`]: tairix_kernel_core::IrqParkWaiter
//!
...
//! [`tairix_kernel_irq::IrqTable::fire`] and the re-arm the park path drives.
//!
//! # Why the crate root, not the `riscv64` port module
//!
//! The bridge is generic over [`tairix_arch_riscv64::plic::PlicMmio`], so it
...
use tairix_arch_riscv64::plic::{PlicController, PlicMmio};
...
/// [`tairix_arch_riscv64::plic::PlicError`] onto [`MaskError::OutOfRange`].

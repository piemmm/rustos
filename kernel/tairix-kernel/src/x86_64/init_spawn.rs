...
//!   [`ArchAddressSpace::new_process_root`]: the higher-half kernel window
//!   the trampoline established plus the direct physical map the boot path
//!   sized from the discovered memory map, and no identity map. The seam
//!   switches to it while building the image, so everything it then touches
//!   — the page-table walk's table recovery, the live allocator's image
//!   frames, the scheduler's LAPIC register writes — is reached either in
//!   the kernel image's own window or through that map.
...
//!   ([`ConfiguredPhysMap`]), whose extent is the discovered RAM.
...
use crate::x86_64::spawn_producer::{page_table_source, ConfiguredPhysMap};
...
impl InitSpawn for X86_64InitSpawn {
    fn spawn_init(&self, ctx: &'static (dyn InitSpawnCtx + Sync)) {
        ...
        // Build PID 1's PML4: the higher-half kernel window and the direct
        // physical map, through which the page-table walk's table recovery,
        // the live allocator's image frames, and the scheduler's LAPIC
        // register writes all resolve under PID 1's CR3 (the production
        // `kernel_main` drain runs on PID 1's root once `pre_resume` has
        // reloaded it). Fail closed on frame exhaustion.
        let Some(arch) = ArchAddressSpace::new_process_root(table_frames) else {
            return;
        };
        ...
        // stack, the per-CPU `swapgs` TLS, the page-table pool, the live
        // frame allocator's frames, the heap, the dispatch path, and every
        // LAPIC access stay mapped across the CR3 switch — exactly
        // `AddressSpace::switch`'s contract. Called once on the boot CPU.
        // The `admit_init` contract requires the image be built into the
        // active space, so the switch precedes `spawn_image` below.
        unsafe { arch.switch() };

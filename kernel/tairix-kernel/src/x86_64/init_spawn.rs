...
//!   unmapped in every root at once, so a kernel-stack overrun faults under
//!   PID 1's CR3 without this seam touching its PML4.
...
use tairix_arch_x86_64::paging::{activate_user_root, AddressSpace as ArchAddressSpace};
...
impl InitSpawn for X86_64InitSpawn {
    fn spawn_init(&self, ctx: &'static (dyn InitSpawnCtx + Sync)) {
        ...
        // frame exhaustion.
        let Some(arch) = ArchAddressSpace::new_identity_window(table_frames) else {
            return;
        };

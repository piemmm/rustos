...
impl InitSpawn for X86_64InitSpawn {
    fn spawn_init(&self, ctx: &'static (dyn InitSpawnCtx + Sync)) {
        ...
        let kernel_stack = guarded_kernel_stack(&mut arch, ctx.frames(), IDENTITY_LIMIT);
        ...
        // identical to the one the image build used (the low identity and the
        // higher-half kernel window are both mapped under PID 1's CR3). No

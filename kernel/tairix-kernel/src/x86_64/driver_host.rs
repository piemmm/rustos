...
//! [`DriverHost::mmio_mapper`]: tairix_abi::DriverHost::mmio_mapper
//! [`DriverHost::virtio_host`]: tairix_abi::DriverHost::virtio_host

use tairix_crypto::Ed25519PublicKey;
use tairix_drvhost::{DriverSpawner, Host, HostConfig, ImageSource};
use tairix_kernel_irq::{IrqTable, IrqWaiter};
use tairix_kernel_mem::{FrameAllocator, MmioMap, PageTable, PhysMap, VirtAddr};
use tairix_kernel_sec::captable::TaskCapabilities;
use tairix_kernel_virtio::kernel_mmio::KernelMmioMapper;
use tairix_kernel_virtio::virtio_factory::{KernelVirtioFactory, KernelVirtioFactoryConfig};
use tairix_log::Sink;

/// Borrowed boot resources [`run_with_driver_host`] needs to assemble an
/// in-kernel [`Host`] that serves a loaded driver both an
/// [`MmioMapper`](tairix_abi::MmioMapper) and a per-driver
/// [`VirtioHost`](tairix_abi::driver::VirtioHost).
///
...
pub struct DriverHostConfig<'k, 'p, P: PageTable> {
    ...
    pub irq_handle: tairix_abi::IrqHandle,
...
/// [`mmio_mapper`](tairix_abi::DriverHost::mmio_mapper) maps register
/// windows through the capability-gated [`KernelMmioMapper`] and whose
/// [`virtio_host`](tairix_abi::DriverHost::virtio_host) factory mints a
...
mod tests {
    ...
    fn driver_without_mmio_cap_is_refused_fail_closed() {
        ...
        run_with_driver_host(config, HostPageTable::new, |host| {
            ...
            assert!(matches!(
                err,
                tairix_drvhost::HostError::DriverRegisterFailed(_)
            ));
        });

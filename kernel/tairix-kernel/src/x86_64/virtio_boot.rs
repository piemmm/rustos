...
use tairix_abi::{IrqHandle, MsiMessage};
...
use tairix_kernel_virtio::virtio_pci_walk::{provision_virtio_pci, VirtioPciWalkError};
...
pub struct VirtioBootConfig<'k, 'p, P: PageTable> {
    ...
    /// `tairix_arch_x86_64::irq::msi_message`) from the vector the
...
/// live [`Host`] whose [`virtio_host`](tairix_abi::DriverHost::virtio_host)
...
mod tests {
    ...
    /// opaque here (the arch encoding is tested in `tairix-arch-x86_64`);

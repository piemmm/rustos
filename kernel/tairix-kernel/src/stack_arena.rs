...
//! ([`tairix_arch_aarch64::paging::AddressSpace::split_block`]) and unmaps
...
//! heap-backed [`tairix_kernel_core::BoxStack`] (its software poison-canary)
...
use tairix_kernel_core::{KernelStack, KTHREAD_STACK_BYTES};
use tairix_kernel_mem::{Frame, FrameAllocator, PhysAddr};
use tairix_sync::SpinLock;

/// Width of a stack's guard region, in bytes: one 4 KiB page.
///
/// Matches [`tairix_kernel_core::BoxStack`]'s guard so the arena form and
...
/// [`tairix_arch_aarch64::paging::AddressSpace::split_block`] requires to
...
const STACK_VA_OFFSET: u64 = tairix_arch_x86_64::paging::KERNEL_VMA_BASE;
...
/// software-canary [`tairix_kernel_core::BoxStack`] — never an unguarded
/// stack.
///
...
impl StackArena {
    ...
    /// software-canary [`tairix_kernel_core::BoxStack`]).
    ///
...
/// dropping the caller to the software-canary [`tairix_kernel_core::BoxStack`]
...
static SHRINK_FRAMES: tairix_sync::Once<&'static FrameAllocator> = tairix_sync::Once::new();

...
fn window_map_cacheable_chunks_maps_blocks_into_one_contiguous_window() {
    ...
    for (i, &want) in expect.iter().enumerate() {
        ...
        assert!(!flags.contains(MapFlags::NO_CACHE));
        assert!(!flags.contains(MapFlags::DMA_COHERENT));
...
fn window_map_chunks_maps_a_dma_region_coherent_on_every_page() {
    ...
    let mut win = window(16);
    let region = win
        .map_chunks_into(&mut space, &[(0x1000_0000, 2)], SharedMemory::DmaCoherent)
        .expect("chunk maps");
    let base = region.virt().as_u64();
    for i in 0..2u64 {
        ...
        let (_, flags) = space
            .translate(Page::from_addr(va).unwrap())
            .expect("window page mapped");
        assert!(flags.contains(MapFlags::DMA_COHERENT), "page {i}");
        assert!(flags.contains(MapFlags::READ | MapFlags::WRITE | MapFlags::USER));
        assert!(!flags.contains(MapFlags::EXEC));
    }
}
...
fn window_map_cacheable_chunks_rejects_a_bad_chunk_list() {
    ...
    // Empty list.
    assert_eq!(
        win.map_chunks_into(&mut space, &[], SharedMemory::Cacheable)
            .err(),
        Some(MmioError::InvalidRegion)
    );
    // A zero-length chunk.
    assert_eq!(
        win.map_chunks_into(&mut space, &[(0x1000_0000, 0)], SharedMemory::Cacheable)
            .err(),
        Some(MmioError::InvalidRegion)
    );
    // A misaligned chunk base.
    assert_eq!(
        win.map_chunks_into(&mut space, &[(0x1000_0800, 1)], SharedMemory::Cacheable)
            .err(),
        Some(MmioError::InvalidRegion)
    );

...
//! bare-metal target (`target_os = "none"`), so the shared standard-streams
//! [`Tty`] implementation (`src/stream.rs`) compiles only where the userland
//! runtime it marshals through exists.
//!
...
//! xtask cfg-check` stays clean. It mirrors `userland/apps/top/build.rs`.

//! The built-in 5×7 monospace glyph atlas.
//!
//! Each glyph is [`GLYPH_HEIGHT`] rows; the low [`GLYPH_WIDTH`] bits of every
...
//! (space through `~`). A character outside that range is rendered by the
//! caller's fallback glyph, never by reading out of bounds.

/// One glyph: [`GLYPH_HEIGHT`] rows, low [`GLYPH_WIDTH`] bits each.
...
/// The first character the atlas covers (space).
pub const FIRST_CHAR: char = ' ';

/// The last character the atlas covers (tilde).
pub const LAST_CHAR: char = '~';

/// Printable-ASCII glyphs, indexed by `code point - FIRST_CHAR`.
pub const GLYPHS: [Glyph; (LAST_CHAR as usize) - (FIRST_CHAR as usize) + 1] = [
    // ' ' (0x20)
    [

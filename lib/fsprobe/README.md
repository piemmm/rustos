...
filesystem (RustFS / ext4 / FAT32), which stable 16-byte identity it
...
import their shared constants and derivations from it (the RustFS
...
- `probe(head)` — recognise a supported filesystem from an extent's
  leading `PROBE_HEAD_LEN` bytes, in a fixed, documented order (RustFS,

...
/// Syndrome members per stripe: RAID5 reserves one member's chunk per
/// stripe for the parity (XOR) of the others, so a stripe carries
/// `n - PARITY_MEMBERS` data chunks and survives that many members being
/// lost.
pub(crate) const PARITY_MEMBERS: usize = 1;
...
impl<B: Block> ParityArray<'_, B> {
    ...
    pub fn health(&self) -> ArrayHealth {
        crate::health::parity_health(self.members.iter().map(ParityMember::state), PARITY_MEMBERS)
    }
...
impl<B: Block> ParityArray<'_, B> {
    ...
    /// # Errors
    ///
    /// [`ParityError::CursorOutOfRange`] if a cursor names a block outside the
    /// array. The array is left exactly as it was, so the caller can proceed
    /// from the fresh-start position.
    pub fn restore_progress(&mut self, progress: ArrayProgress) -> Result<(), ParityError> {
        if !progress.fits_span(self.per_member_blocks) {
            return Err(ParityError::CursorOutOfRange);
        }

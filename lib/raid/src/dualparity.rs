...
/// Syndrome members per stripe: RAID6 reserves two members' chunks per
/// stripe — the P (XOR) and Q (Reed-Solomon) syndromes — so a stripe
/// carries `n - PARITY_MEMBERS` data chunks and survives that many members
/// being lost.
pub(crate) const PARITY_MEMBERS: usize = 2;
...
impl<'a, B: Block> DualParityArray<'a, B> {
    ...
    /// Wrap already-prepared members and scratch as a dual-parity view
...
impl<B: Block> DualParityArray<'_, B> {
    ...
    pub fn health(&self) -> ArrayHealth {
        crate::health::parity_health(
            self.members.iter().map(DualParityMember::state),
            PARITY_MEMBERS,
        )
    }

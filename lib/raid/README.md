...
TAIRiX **RAID composition** engines: fault-aware virtual block devices that
compose child block endpoints through the public block seam
(`plans/FIX-IO.md` IO6; `docs/src/lib/raid.md`).
...
[`tairix_abi::driver::block::Block`](../abi): it composes several
...
## Why it is a shared crate

Composition is device-agnostic arithmetic over the generic block seam: this
crate composes devices it is *handed* as `Block` implementations and never
reaches a device itself, so it carries no bring-up, firmware, quirk, or
register logic and is no device's support code. It sits in `lib/` because two
independent consumers compose several block devices as one and must not each
carry their own copy of the arithmetic: the autoloaded RAID composer driver
(`drivers/storage/raid`), which assembles members discovered from their
superblocks and serves each array as one published block-service node, and the
native filesystem (`drivers/filesystem/arxfs`), whose multi-device volumes
drive the same engines directly. A copy in either driver would be unreachable
from the other, since one driver crate may not depend on another.
...
ceiling and holds only a borrow. It depends only on `lib/abi` and the shared
on-disk metadata crate `lib/raidmeta`; a member is reached through the `Block`
trait the serve process is handed, never a sibling driver crate.

...
//! bare-metal target (`target_os = "none"`), so the production transport
//! (`src/rt.rs`) compiles only where the userland runtime it marshals
//! through exists.
//!
...
//! xtask cfg-check` stays clean. It mirrors `lib/curses/build.rs`.

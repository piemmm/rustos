...
//! primary presses on the *same* item close enough together in time. The
...
//! shared pixel→index hit-test) and a monotonic timestamp (the kernel monotonic
//! clock, which needs no capability), and performs the activation itself under
...
pub enum ClickKind {
    ...
    Single,
    /// The second press of a pair on the same item within the interval: the
    /// caller activates the item (descend / launch a bundle / open a file).
    Double,
}

/// One remembered primary press: the item it landed on and when.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
struct LastClick {
...
/// remembered press, so only *consecutive* presses on the *same* item can pair.
...
impl DoubleClickTracker {
    ...
    /// Register a primary press on the item at `index` at monotonic time
    /// `now_ns`, using the default [`DOUBLE_CLICK_INTERVAL_NS`] window, and
    /// report whether it completes a double-click.
    #[must_use]
    pub fn register(&mut self, now_ns: u64, index: usize) -> ClickKind {
        self.register_within(now_ns, index, DOUBLE_CLICK_INTERVAL_NS)
    }
    ...
    pub fn register_within(&mut self, now_ns: u64, index: usize, interval_ns: u64) -> ClickKind {
        if let Some(prev) = self.last {
            // Pair only a press on the same item that follows the previous one
            // within the window. `now_ns >= prev.at_ns` guards a
            // non-monotonic reading (a clock that appeared to step back): such
            // a press fails closed to a fresh single rather than a spurious
            // double.
            if prev.index == index && now_ns >= prev.at_ns && now_ns - prev.at_ns <= interval_ns {
                self.last = None;
...
#[cfg(test)]
mod tests {
    use super::{ClickKind, DoubleClickTracker, DOUBLE_CLICK_INTERVAL_NS};

    #[test]
    fn a_lone_press_is_a_single_click() {
        let mut tracker = DoubleClickTracker::new();
        assert_eq!(tracker.register(0, 3), ClickKind::Single);
    }

    #[test]
    fn two_quick_presses_on_the_same_item_are_a_double_click() {
        let mut tracker = DoubleClickTracker::new();
        assert_eq!(tracker.register(1_000, 3), ClickKind::Single);
        assert_eq!(
            tracker.register(1_000 + DOUBLE_CLICK_INTERVAL_NS / 2, 3),
            ClickKind::Double
        );
    }
    ...
    fn a_press_exactly_at_the_interval_still_pairs() {
        let mut tracker = DoubleClickTracker::new();
        assert_eq!(tracker.register(0, 0), ClickKind::Single);
        assert_eq!(
            tracker.register(DOUBLE_CLICK_INTERVAL_NS, 0),
            ClickKind::Double
        );
    }
    ...
    fn a_slow_second_press_is_a_fresh_single_not_a_double() {
        ...
        assert_eq!(tracker.register(0, 3), ClickKind::Single);
        assert_eq!(
            tracker.register(DOUBLE_CLICK_INTERVAL_NS + 1, 3),
            ClickKind::Single
        );
    }
    ...
    fn a_quick_press_on_a_different_item_is_a_single() {
        let mut tracker = DoubleClickTracker::new();
        assert_eq!(tracker.register(0, 3), ClickKind::Single);
        assert_eq!(tracker.register(1, 4), ClickKind::Single);
    }
    ...
    fn a_double_click_consumes_both_presses() {
        let mut tracker = DoubleClickTracker::new();
        assert_eq!(tracker.register(0, 3), ClickKind::Single);
        assert_eq!(tracker.register(1, 3), ClickKind::Double);
        // A third quick press begins a fresh single, never a second double.
        assert_eq!(tracker.register(2, 3), ClickKind::Single);
        assert_eq!(tracker.register(3, 3), ClickKind::Double);
    }
    ...
    fn a_reset_breaks_the_pair() {
        ...
        assert_eq!(tracker.register(1, 3), ClickKind::Single);
    }

    #[test]
    fn a_backwards_clock_reading_fails_closed_to_a_single() {
        let mut tracker = DoubleClickTracker::new();
        assert_eq!(tracker.register(10_000, 3), ClickKind::Single);
        // A reading before the remembered press must not pair (it would
        // otherwise underflow the interval test); it is a fresh single.
        assert_eq!(tracker.register(9_000, 3), ClickKind::Single);
        // And that fresh press is now the remembered one: a proper follow-up
        // pairs against it.
        assert_eq!(tracker.register(9_500, 3), ClickKind::Double);
    }

    #[test]
    fn a_custom_interval_is_honoured() {
        let mut tracker = DoubleClickTracker::new();
        assert_eq!(tracker.register_within(0, 1, 100), ClickKind::Single);
        assert_eq!(tracker.register_within(50, 1, 100), ClickKind::Double);
        assert_eq!(tracker.register_within(200, 1, 100), ClickKind::Single);
        assert_eq!(tracker.register_within(400, 1, 100), ClickKind::Single);
    }
}

...
fn a_unique_publication_gains_an_nsec_asserting_what_it_does_not_hold() {
    ...
    let emit = engine
        .on_message(at(5000), &asked, peer(9), PORT, &mut rng, &mut buf)
        .expect("answers");
...
fn a_peer_claiming_our_live_name_renames_us_and_restarts_probing() {
    ...
    let mut buf = [0u8; BUF];
    assert!(engine
        .on_message(at(5000), &theirs, peer(9), PORT, &mut rng, &mut buf)
        .is_none());
...
fn the_same_data_at_our_name_is_not_a_conflict() {
    ...
    let mut buf = [0u8; BUF];
    engine.on_message(at(5000), &echo, peer(9), PORT, &mut rng, &mut buf);
...
fn the_rename_budget_runs_out_and_the_name_fails_closed_to_unpublished() {
    ...
    assert!(engine
        .on_message(at(9000), &asked, peer(9), PORT, &mut rng, &mut buf)
        .is_none());
...
fn losing_a_simultaneous_probe_defers_rather_than_races() {
    ...
    let mut buf = [0u8; BUF];
    engine.on_message(at(10), &theirs, peer(9), PORT, &mut rng, &mut buf);
...
fn a_legacy_resolver_gets_its_question_echoed_and_a_short_ttl() {
    ...
    let mut buf = [0u8; BUF];
    let emit = engine
        .on_message(at(5000), &asked, peer(9), 41234, &mut rng, &mut buf)
...
fn a_known_answer_the_asker_already_holds_is_not_repeated() {
    ...
    assert!(engine
        .on_message(at(5100), &asked, peer(9), PORT, &mut rng, &mut buf)
        .is_some());
}
...
fn a_name_still_being_probed_for_is_not_answered_with() {
    ...
    assert!(
        engine
            .on_message(at(10), &asked, peer(9), PORT, &mut rng, &mut buf)
            .is_none(),
...
fn a_query_from_off_link_is_never_answered() {
    ...
    let asked = query(&name("printer.local"), QuestionType::Any, true);
    let mut buf = [0u8; BUF];
    let off_link = IpAddr::V4(Ipv4Addr::new(203, 0, 113, 7));
    assert!(engine
        .on_message(at(5000), &asked, off_link, PORT, &mut rng, &mut buf)
        .is_none());
...
fn a_response_from_off_link_never_reaches_the_cache() {
    ...
    let mut buf = [0u8; BUF];
    let off_link = IpAddr::V6(Ipv6Addr::new(0x2001, 0xdb8, 0, 0, 0, 0, 0, 1));
    engine.on_message(at(0), &theirs, off_link, PORT, &mut rng, &mut buf);
...
fn a_record_is_multicast_at_most_once_a_second() {
    ...
    let mut buf = [0u8; BUF];
    let mut rng = highest();
    engine.on_message(at(10_000), &asked, peer(9), PORT, &mut rng, &mut buf);
    ...
    engine.on_message(at(10_200), &asked, peer(9), PORT, &mut rng, &mut buf);
    ...
    engine.on_message(at(12_000), &asked, peer(9), PORT, &mut rng, &mut buf);
    let due = engine.next_deadline().expect("scheduled again");
...
fn defending_our_own_name_is_never_charged_to_a_budget() {
    ...
    let mut buf = [0u8; BUF];
    for _ in 0..200 {
        engine.on_message(at(5000), &unicast, peer(9), PORT, &mut rng, &mut buf);
    }
    ...
    let emit = engine
        .on_message(at(5000), &probe, peer(9), PORT, &mut rng, &mut buf)
...
fn withdrawing_sends_goodbyes_and_then_publishes_nothing() {
    ...
    let mut buf = [0u8; BUF];
    assert!(engine
        .on_message(at(6000), &asked, peer(9), PORT, &mut rng, &mut buf)
        .is_none());
}
...
fn a_query_carries_what_we_already_know_so_responders_can_stay_quiet() {
    ...
    let mut buf = [0u8; BUF];
    engine.on_message(at(0), &theirs, peer(9), PORT, &mut rng, &mut buf);
...
fn another_responder_answering_first_drops_our_pending_answer() {
    ...
    engine.on_message(at(10_010), &theirs, peer(8), PORT, &mut rng, &mut buf);
    assert!(

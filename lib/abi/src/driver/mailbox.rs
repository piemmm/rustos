...
//! BCM2711 `VideoCore` client — `AGENTS.md` §2.20 carve-out); the host hands
...
//! accessor can name it without inverting the dependency direction
//! (`AGENTS.md` §3): `lib/vcmailbox` depends on `lib/abi`, never the reverse.
...
/// the same width so the buffer shape is defined exactly once (`AGENTS.md`
/// §2.2).
...
/// of board addresses (`AGENTS.md` §2.20) and of any `kernel/*` dependency
/// (`AGENTS.md` §17.4).
pub trait MailboxChannel {
    ...
    /// must treat the exchange as failed (fail closed — `AGENTS.md` §5.4).
    ///
    /// # Errors
    ///

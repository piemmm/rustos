...
mod tests {
    use super::{
        put_i32, put_i64, put_u16, put_u32, put_u64, read_i32, read_i64, read_u16, read_u32,
        read_u64,
    };

    #[test]
    fn i32_round_trips_negative_at_offset() {
        let mut buf = [0u8; 8];
        put_i32(&mut buf, 2, -2);
        assert_eq!(read_i32(&buf, 2), -2);
        assert_eq!(&buf[2..6], &[0xFE, 0xFF, 0xFF, 0xFF]);
    }

...
/// A translucent pixel of alpha `a`.
fn veil(a: u8) -> Pixel {
    Color::rgba(10, 20, 30, a).premultiply()
}

#[test]
fn a_filled_surface_knows_its_own_alpha_without_reading_a_pixel() {
    assert_eq!(Surface::new(4, 4).expect("surface").alpha_floor(), 0);
    assert_eq!(
        Surface::filled(4, 4, veil(204))
            .expect("surface")
            .alpha_floor(),
        204
    );
...
#[test]
fn a_borrow_reads_as_transparent_until_it_is_settled() {
    let mut surface = Surface::filled(4, 4, veil(204)).expect("surface");
    let _ = surface.row_span_mut(1, 0, 4).expect("row admitted");
    assert_eq!(
        surface.alpha_floor(),
        0,
        "a write nobody has read back may have laid anything down"
    );
    surface.settle_alpha_floor();
    assert_eq!(surface.alpha_floor(), 204, "and nothing was laid down");
}

#[test]
fn a_partial_write_lowers_the_floor_to_what_it_wrote() {
    let mut surface = Surface::filled(8, 8, veil(204)).expect("surface");
    surface.set(3, 3, veil(64));
    surface.settle_alpha_floor();
    assert_eq!(surface.alpha_floor(), 64);

    // Painting over the one translucent pixel does not lift the floor: the
    // pixel that held the minimum may be anywhere, and only a full retake can
    // say. The bound stays sound, which is the whole contract.
    surface.set(3, 3, veil(255));
    surface.settle_alpha_floor();
    assert_eq!(surface.alpha_floor(), 64);
}

#[test]
fn the_floor_is_retaken_in_full_once_a_surface_s_worth_has_been_written() {
    let mut surface = Surface::filled(8, 8, veil(204)).expect("surface");
    surface.set(3, 3, veil(64));
    surface.settle_alpha_floor();
    assert_eq!(surface.alpha_floor(), 64);

    // Eight opaque rows bring the area written since the last retake up to the
    // surface's own, so that settle reads every pixel and finds the 64 painted
    // out. Nothing before it does, however many rows have been read back.
    for row in 0..8 {
        surface.fill_rect(0, row, 8, 1, Color::rgba(0, 0, 0, 255));
        let before = surface.alpha_floor();
        surface.settle_alpha_floor();
        if row < 7 {
            assert_eq!(surface.alpha_floor(), 64, "row {row} retook early");
        }
        assert_eq!(before, 0, "an unsettled borrow claims nothing");
    }
    assert_eq!(surface.alpha_floor(), 255);
}

#[test]
fn a_surface_built_from_bytes_reads_its_own_pixels_once_asked() {
    let rgba: alloc::vec::Vec<u8> = (0..4 * 4)
        .flat_map(|i| [255, 255, 255, if i == 5 { 100 } else { 255 }])
        .collect();
    let mut surface = Surface::from_rgba8(4, 4, &rgba).expect("surface");
    assert_eq!(surface.alpha_floor(), 0, "nothing is claimed unread");
    surface.settle_alpha_floor();
    assert_eq!(surface.alpha_floor(), 100);
}

#[test]
fn a_band_pass_accounts_for_every_row_it_hands_out() {
    let mut surface = Surface::filled(4, 6, veil(204)).expect("surface");
    for mut band in surface.row_bands_mut(2..4, 1) {
        let rows = band.rows();
        for y in rows {
            if let Some((_, span)) = band.row_span_mut(y, 0, 4) {
                span.fill(veil(12));
            }
        }
    }
    surface.settle_alpha_floor();
    assert_eq!(surface.alpha_floor(), 12);
}

#[test]
fn two_surfaces_with_the_same_pixels_are_equal_however_tight_their_floors() {
    let tight = Surface::filled(4, 4, veil(204)).expect("surface");
    let mut loose = Surface::filled(4, 4, veil(204)).expect("surface");
    loose.set(0, 0, veil(204));
    loose.settle_alpha_floor();
    assert_eq!(tight.pixels(), loose.pixels());
    assert_eq!(tight, loose, "the bound is derived, not identity");
}

#[test]
fn a_wiped_surface_claims_nothing() {
    let mut surface = Surface::filled(4, 4, veil(255)).expect("surface");
    assert_eq!(surface.alpha_floor(), 255);
    surface.wipe();
    assert_eq!(surface.alpha_floor(), 0);
}

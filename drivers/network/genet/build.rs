...
//! bare-metal target (`target_os = "none"`), so the GENET driver's `Run`
//! binary (`src/main.rs`) compiles as a freestanding pure-Rust program there
//! and as an inert host stub everywhere else (`plans/NETWORK.md` N14).
//!
...
//! xtask cfg-check` stays clean. It mirrors `drivers/bus/pcie_brcm/build.rs`.

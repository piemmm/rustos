...
//! bare-metal target (`target_os = "none"`), so the virtio-net driver's `Run`
//! binary (`src/main.rs`) compiles as a freestanding pure-Rust program there
//! and as an inert host stub everywhere else (`plans/NETWORK.md` N4d).
//!
...
//! xtask cfg-check` stays clean. It mirrors `drivers/input/virtio_kbd/build.rs`.

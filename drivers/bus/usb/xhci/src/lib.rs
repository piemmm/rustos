...
//! loadable host-controller *driver*: the [`register`] entry, the
//! [`BIND_KEYS`] bind table, and the PCI discovery / BAR / DMA
...
//! (`drivers/* → lib/*` only; the USB analogue of
...
//! user space and does not request `CAP_DRV_KERNEL`.
...
/// vendor-specific driver that names an exact device id,.
...
/// controller — autoloads it without the device id being hard-coded. This `const` is the single source of truth

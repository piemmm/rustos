...
//! inbound-DMA aperture are device-tree-discovered, never compiled-in); a `devmgr`/host composition maps that window,
...
//!
//! No QEMU vertical exists — QEMU models no Pi USB timing — so the host tests prove the composition and its
...
/// carves a controller's DMA region.
...
/// `dma-ranges` aperture the hardware tree discovered): the carved region's device-visible address
...
///   `outbound_window` (fail closed), plus any error
...
/// the hardware tree as a bindable child [`HwNode`] (
/// `plans/PI.md` Increment C / driver-traits "Autonomous floor bring-up
/// entry").
///
/// `register` is *reactive*: the host instantiates the
...
/// chain over the [`DriverHost`] contract alone (no `kernel/*` dependency): [`map_controller`] (mapping the BAR through
...
/// tier through an onboard hub), and finally
/// [`DriverHost::emit_node`] of the enumerated device.
///
/// This crate stays board-neutral: it never names the
...
pub fn map_controller(
    ...
    // generic allocation contract (the C-1 split that
    ...
    // shared bus-driver scan (one definition in
    ...
    // fail-closed refusal, never a silent truncation. The slab is dropped (reclaimed) on the early return.

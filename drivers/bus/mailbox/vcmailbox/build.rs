...
//! bare-metal target (`target_os = "none"`), so the `VideoCore` mailbox service
//! driver's `Run` binary (`src/main.rs`) compiles as a freestanding pure-Rust
//! program there and as an inert host stub everywhere else (`plans/PI.md` P10).
//!
...
//! `drivers/input/usb_kbd/build.rs` (`AGENTS.md` §2.2).

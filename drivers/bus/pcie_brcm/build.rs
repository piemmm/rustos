...
//! bare-metal target (`target_os = "none"`), so the BCM2711 PCIe bus driver's
//! `Run` binary (`src/main.rs`) compiles as a freestanding pure-Rust program
//! there and as an inert host stub everywhere else (`plans/PI.md` P10 D5b.2b).
//!
...
//! `drivers/bus/mailbox/vcmailbox/build.rs` (`AGENTS.md` §2.2).

...
//! bare-metal target (`target_os = "none"`), so the RAID array-composer
//! driver's `Run` binary (`src/main.rs`) compiles as a freestanding pure-Rust
//! program there and as an inert host stub everywhere else
...
//! `drivers/storage/raid_member/build.rs`.

...
//! (`docs/src/filesystem/rustfs-spec.md` §12, §15.8).
//!
...
/// Range start (inclusive) reserved for `rustfs` scrub event identifiers.
pub const RUSTFS_RANGE_START: u32 = 12_000;
/// Range end (exclusive) reserved for `rustfs` scrub event identifiers.
pub const RUSTFS_RANGE_END: u32 = 13_000;
...
const _: () = {
    ...
    assert!(SCRUB_PAUSED.0 >= RUSTFS_RANGE_START && SCRUB_PAUSED.0 < RUSTFS_RANGE_END);
    assert!(SCRUB_DENIED.0 >= RUSTFS_RANGE_START && SCRUB_DENIED.0 < RUSTFS_RANGE_END);
};

/// How much work one [`crate::RustFs::scrub`] call may do before persisting a
...
/// The structured outcome of a scrub pass (`docs/src/filesystem/rustfs-spec.md`
...
impl ScrubReport {
    ...
    pub(crate) fn log_outcome(&self, sink: &dyn Sink) {
        let (id, level, message) = if !self.complete {
            ...
            (SCRUB_FAULTS_FOUND, Level::Warn, "rustfs scrub found faults")
        } else {
            (SCRUB_CLEAN, Level::Info, "rustfs scrub clean")
        };
        log(
            sink,
            &Event {
        ...
        if !caps.holds(CapabilityId::FS_MOUNT) {
            log(
                ...
                &Event {
                    level: Level::Warn,
                    id: SCRUB_DENIED,
                    message: "rustfs scrub denied: missing CAP_FS_MOUNT",
                    fields: &[],
                },
            );
...
impl<B: Block> RustFs<B> {
    ...
    /// reverse-reference trees (`docs/src/filesystem/rustfs-spec.md` §9). The

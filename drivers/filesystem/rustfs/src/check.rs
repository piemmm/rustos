//! Offline check and rescue (`docs/src/filesystem/rustfs-spec.md` §12, §15.9).
//!
...
//! `rustfs` `12000..13000` range (`crate::scrub`).
...
const _: () = {
    ...
    assert!(
        CHECK_UNRECOVERABLE.0 >= RUSTFS_RANGE_START && CHECK_UNRECOVERABLE.0 < RUSTFS_RANGE_END
    );
    assert!(CHECK_DENIED.0 >= RUSTFS_RANGE_START && CHECK_DENIED.0 < RUSTFS_RANGE_END);
    assert!(RESCUE_COMPLETE.0 >= RUSTFS_RANGE_START && RESCUE_COMPLETE.0 < RUSTFS_RANGE_END);
    assert!(RESCUE_NO_ROOT.0 >= RUSTFS_RANGE_START && RESCUE_NO_ROOT.0 < RUSTFS_RANGE_END);
    assert!(RESCUE_DENIED.0 >= RUSTFS_RANGE_START && RESCUE_DENIED.0 < RUSTFS_RANGE_END);
};

/// The structured outcome of an offline [`RustFs::check`]
/// (`docs/src/filesystem/rustfs-spec.md` §12). It carries the shared
    ...
    fn log_outcome(&self, sink: &dyn Sink) {
        let (id, level, message) = if self.unrecoverable_findings != 0 {
            (
                CHECK_UNRECOVERABLE,
                Level::Warn,
                "rustfs check found unrecoverable structural findings",
            )
        } else if self.made_repairs() {
            (
                CHECK_REPAIRED,
                Level::Info,
                "rustfs check repaired structural findings",
            )
        } else {
            (CHECK_CLEAN, Level::Info, "rustfs check clean")
        };
        log(
            sink,
            &Event {
...
/// The structured outcome of a [`RustFs::rescue`] pass
    ...
    fn log_outcome(&self, sink: &dyn Sink) {
        let (id, level, message) = if self.found_root() {
            (
                RESCUE_COMPLETE,
                Level::Info,
                "rustfs rescue extracted readable files from a recovered root",
            )
        } else {
            (
                RESCUE_NO_ROOT,
                Level::Warn,
                "rustfs rescue found no valid root",
            )
        };
        log(
            sink,
...
/// Where [`RustFs::rescue`] delivers the file data it recovers. The driver
...
/// ever delivered (`docs/src/filesystem/rustfs-spec.md` §6, §12).
...
impl<B: Block> RustFs<B> {
        ...
        if !caps.holds(CapabilityId::FS_MOUNT) {
            log(
                ...
                &Event {
                    level: Level::Warn,
                    id: CHECK_DENIED,
                    message: "rustfs check denied: missing CAP_FS_MOUNT",
                    fields: &[],
                },
            );
    ...
    /// directory tree no longer reaches (`docs/src/filesystem/rustfs-spec.md`
    ...
    pub fn rescue(
        ...
        if !caps.holds(CapabilityId::FS_MOUNT) {
            log(
                ...
                &Event {
                    level: Level::Warn,
                    id: RESCUE_DENIED,
                    message: "rustfs rescue denied: missing CAP_FS_MOUNT",
                    fields: &[],
                },
            );
            return Err(DriverError::PermissionDenied);
        ...
        // plaintext); it fails closed on a wrong key or a non-rustfs device.

...
//! (`docs/src/filesystem/rustfs-spec.md` §11, §15.11).
//!
...
use crate::scrub::{ScrubBudget, ScrubReport, RUSTFS_RANGE_END, RUSTFS_RANGE_START};
use crate::{rd_u64, wr_u64, RustFs, MAX_BLOCK_SIZE};
...
const _: () = {
    ...
    assert!(HEALTH_FAILING.0 >= RUSTFS_RANGE_START && HEALTH_FAILING.0 < RUSTFS_RANGE_END);
    assert!(
        HEALTH_SCRUB_TRIGGERED.0 >= RUSTFS_RANGE_START
            && HEALTH_SCRUB_TRIGGERED.0 < RUSTFS_RANGE_END
    );
    assert!(HEALTH_DENIED.0 >= RUSTFS_RANGE_START && HEALTH_DENIED.0 < RUSTFS_RANGE_END);
};
...
/// The volume's health classification (`docs/src/filesystem/rustfs-spec.md`
...
impl HealthThresholds {
    /// The fixed thresholds [`RustFs::health`] classifies against. Chosen so
...
/// The structured outcome of a [`RustFs::health`] pass
/// (`docs/src/filesystem/rustfs-spec.md` §11), mirroring [`ScrubReport`] /
        ...
        if self.scrub.is_some() {
            log(
                ...
                &Event {
                    ...
                    message: "rustfs health triggered a scrub",
                    fields: &[],
                },
            );
        }
        let (level, id, message) = match self.state {
            HealthState::Healthy => (Level::Info, HEALTH_OK, "rustfs health: volume healthy"),
            HealthState::Degraded => (
                Level::Warn,
                HEALTH_DEGRADED,
                "rustfs health: volume degraded",
            ),
            HealthState::Failing => (
                Level::Error,
                HEALTH_FAILING,
                "rustfs health: volume failing",
            ),
        };
        log(
            sink,
...
/// (critical single-device health, `docs/src/filesystem/rustfs-spec.md` §11).
...
impl<B: Block> RustFs<B> {
    /// Run a device-health pass (`docs/src/filesystem/rustfs-spec.md` §11).
    ///
        ...
        if !caps.holds(CapabilityId::FS_MOUNT) {
            log(
                ...
                &Event {
                    level: Level::Warn,
                    id: HEALTH_DENIED,
                    message: "rustfs health denied: missing CAP_FS_MOUNT",
                    fields: &[],
                },
            );
    ...
    /// zeroed fault history (`docs/src/filesystem/rustfs-spec.md` §11 mkfs

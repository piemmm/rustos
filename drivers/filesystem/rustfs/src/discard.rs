...
//! (`docs/src/filesystem/rustfs-spec.md` §11, §15.10).
//!
...
use crate::scrub::{RUSTFS_RANGE_END, RUSTFS_RANGE_START};
use crate::{RustFs, RING_BLOCKS};
...
const _: () = {
    ...
    assert!(TRIM_UNSUPPORTED.0 >= RUSTFS_RANGE_START && TRIM_UNSUPPORTED.0 < RUSTFS_RANGE_END);
    assert!(TRIM_DENIED.0 >= RUSTFS_RANGE_START && TRIM_DENIED.0 < RUSTFS_RANGE_END);
};
    ...
    fn log_outcome(&self, sink: &dyn Sink) {
        let (level, id, message) = if !self.supported {
            (
                Level::Info,
                TRIM_UNSUPPORTED,
                "rustfs trim: device has no discard support; queue drained",
            )
        } else if self.blocks_discarded > 0 {
            (
                Level::Info,
                TRIM_DISCARDED,
                "rustfs trim discarded freed ranges",
            )
        } else {
            (Level::Info, TRIM_CLEAN, "rustfs trim: nothing to discard")
        };
        log(
            sink,
            &Event {
        ...
        if !caps.holds(CapabilityId::FS_MOUNT) {
            log(
                ...
                &Event {
                    level: Level::Warn,
                    id: TRIM_DENIED,
                    message: "rustfs trim denied: missing CAP_FS_MOUNT",
                    fields: &[],
                },
            );

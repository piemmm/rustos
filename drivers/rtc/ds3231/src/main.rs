//! The `Run` entry-point binary of the Maxim DS3231 / DS1307 real-time-clock driver,
//! installed as a signed `/System/Drivers/` bundle and **autoloaded into user
//! space** by `devmgr` when a `maxim,ds3231` node is discovered
...
//! `tairix-rt`, never the C ABI. The part needs no bring-up step: it runs
//! from its backup cell whenever it can, and a chip that cannot vouch for
//! itself says so at the first read rather than at load. A start-up failure
//! exits with a reserved fail-closed code, leaving the machine without a
//! local time source rather than wedged. On the host it is an inert stub so
...
mod program {
    ...
    use tairix_drv_rtc_ds3231::Ds3231;
    ...
    fn serve(rtc: &mut Ds3231<&RtDriverHost<RtGrantSyscalls>>) -> i32 {

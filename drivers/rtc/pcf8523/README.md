...
Autoloaded user-space real-time-clock driver for the **NXP PCF8523**, an I²C
calendar chip at bus address `0x68`.

## Supported hardware

Device-tree `compatible`: `nxp`, `pcf8523`. The
...
whenever the oscillator has stopped — a flat backup cell, a first power-on, a
brown-out — and cleared only by a write. While it is set the calendar registers
mean nothing, so `read` answers `Ok(None)`.

Bring-up puts the part in **24-hour** mode with its time circuits running,
because both live in a control register rather than in the calendar block: a
driver that only read them would be trusting whatever a previous owner left
there. It is a read-modify-write, so the board's other settings survive. The
read path still honours a 12-hour field, because the mode bit and the field can
legitimately disagree in the window before the first write.

The part has a backup-cell input and a switch-over circuit, but the cell is an
optional accessory a board may not fit — so `battery_backed` is read from the
control register's battery-switch-over setting rather than asserted from the
part number. A board that disabled switch-over is honestly reported as not
backed.
...
- NXP PCF8523 data sheet, register overview.

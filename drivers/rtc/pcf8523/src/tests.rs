//! Host unit tests for the Pcf8523 driver, driven against the shared
...
use super::{
    register, Pcf8523, BIND_KEYS, CONTROL_1_STOP, CONTROL_1_TWELVE_HOUR, HOUR_PM,
    PCF8523_BUS_ADDRESS, PCF8523_COMPATIBLE, REG_CONTROL_1, REG_CONTROL_3, REG_SECONDS,
...
fn bring_up_puts_the_chip_in_twenty_four_hour_mode_and_running() {
    ...
    Pcf8523::open(&part).expect("binds");
...
fn a_running_chip_reads_back_the_instant_its_registers_hold() {
    let part = MockPart::new();
    let mut chip = Pcf8523::open(&part).expect("binds");
...
fn an_unreachable_part_refuses_bring_up_rather_than_coming_up_blind() {
    let part = MockPart::new();
    part.fail_with(DriverError::NotFound);
    assert_eq!(Pcf8523::open(&part).err(), Some(DriverError::NotFound));
}

#[test]
fn the_battery_report_comes_from_the_boards_own_switch_over_setting() {
    // Switch-over enabled: the board fitted a cell, so the reading survives
    // a power cycle.
    let part = MockPart::new();
    let mut chip = Pcf8523::open(&part).expect("binds");
    part.seed(REG_CONTROL_3, &[0b0000_0000]);
    assert!(chip.status().expect("reads").battery_backed);

    // Switch-over disabled: no evidence of persistence, so none is claimed.
    for mode in [0b1010_0000u8, 0b1100_0000, 0b1110_0000] {
        part.seed(REG_CONTROL_3, &[mode]);
        assert!(
            !chip.status().expect("reads").battery_backed,
            "mode {mode:#010b} disables switch-over"
        );
    }
    assert_eq!(chip.status().expect("reads").precision.secs(), 1);
}
...
fn the_bind_table_names_the_part_and_fits_the_abi_bound() {
    ...
    assert!(PCF8523_COMPATIBLE.len() <= HW_COMPATIBLE_MAX);
    let expected = HwMatchKey::compatible(PCF8523_COMPATIBLE).expect("fits");
    assert_eq!(BIND_KEYS[0].key, expected);
    assert_eq!(PCF8523_BUS_ADDRESS, 0x68);
}

//! The `Run` entry-point binary of the Goldfish real-time-clock driver,
//! installed as a signed `/System/Drivers/` bundle and **autoloaded into user
//! space** by `devmgr` when a `google,goldfish-rtc` node is discovered
//! (`plans/TIMESYNC.md` TS-3).
//!
//! The process owns the counter's register window and serves the RTC class
...
//!   the grants the kernel minted for its matched node (one register window)
//!   and maps it. Every capability and bound is re-checked kernel-side.
//! * `sole_register_window` over the delivered grants, so the counter's
//!   `(base, len)` comes from discovery and never a build-time board
//!   constant.
...
mod program {
    ...
    use tairix_drv_rtc_goldfish::Goldfish;
    ...
    /// Exit code when the delivered grants do not name the single counter
    /// register window this driver needs — an unbound or mis-provisioned
    /// node. A reserved, fail-closed value.
    const EXIT_NO_RESOURCES: i32 = 81;

    /// Exit code when the register window could not be mapped, or the mapped
    /// window is too short to hold the counter pair. A reserved value.
    ...
    /// The capability set the host re-checks before issuing an `mmio_map`
    /// trap, plus the bind privilege needed to create a restricted-sender
    ...
    fn main() -> i32 {
        ...
        let Ok(mut rtc) = Goldfish::new(regs) else {
            return EXIT_BRINGUP_FAILED;
        };
    ...
    /// run the request against the counter, and answer with `call_reply`.
    ///

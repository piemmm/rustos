# RUSTFS-METADATA.md — RustFS extensible file-metadata design brief
...
material to produce a new section of `docs/src/filesystem/rustfs-spec.md`
...
1. Give every RustFS file a general-purpose **extended-attribute** facility — an
   extensible, namespaced `key → value` store per inode — because RustFS today
...
2. Use that facility to **preserve foreign-filesystem metadata** that would
   otherwise be lost when a file is copied onto RustFS — the Acorn/RISC OS
...
RustOS later gains ADFS/AmigaDOS/etc. read-write support, a copy *off* RustFS
back onto the native filesystem must be able to restore the original metadata.

RustFS therefore needs a lossless, general place to keep arbitrary foreign and
...
- Attribute values are **opaque byte strings** to RustFS; RustFS never
...
- No code duplication: the namespaced-key parser, the preset registry, and the
  conversion helpers live **once** in a shared `lib/*` crate so RustFS, the
...
Each RustFS inode gains an optional reference to an **attribute set**: an
ordered collection of `(key, value)` pairs. Small sets are stored inline with
the inode (RustFS already supports inline/packed small-file storage,
rustfs-spec §5); larger sets spill into dedicated COW metadata blocks named
from the inode, exactly as directory blocks and extents are (rustfs-spec §4,
§13). The attribute set is part of the inode's COW image, so setting an
attribute is one atomic transaction and a crash leaves prior-or-new (rustfs-spec
§14).
...
A key is `namespace "." rest`, byte-for-byte case-sensitive (matching RustFS
directory-name comparison, rustfs-spec §13), e.g. `acorn.filetype`,
...
- Attribute keys and values live in the encrypted-metadata domain like filenames
  (rustfs-spec §7); no plaintext attribute leaks on a raw-device read.
...
dest driver    -> store natively if it understands them, else keep as RustFS
                  extended attributes verbatim (lossless round-trip)
```

- **Foreign → RustFS**: the source driver yields preset attributes; RustFS
  stores them verbatim in the matching namespace. Nothing is lost.
- **RustFS → foreign (same family)**: RustFS yields the stored preset
...
- **RustFS → foreign (different/foreign-incapable, e.g. FAT32)**: attributes
...
- Snapshot send/receive (plans/RUSTFS-SNAPSHOT.md §6.3) carries the full
  attribute set so a backup round-trip is lossless.

### 6.3 Archive interop

Archive create/extract (`/System/Libraries/` archive class, charter §16.4) must
carry the normalised preset attributes too, so a file archived on RustFS and
...
- Shared crate: add `lib/fsmeta` (name TBD) holding the namespaced-key grammar +
  ...
  definition shared by RustFS, the foreign-FS drivers, the copy/move tools, the
...
- Docs: new `§21 Extended file metadata` in `docs/src/filesystem/rustfs-spec.md`,
  a row in the §2 mandatory feature table, a stage in §18, plus a registry
  reference page under `docs/src/filesystem/`. Update
  `docs/src/filesystem/rustfs.md` and the ext4/fat32 driver docs to state their
...
3. attributes are encrypted at rest (no plaintext key/value on a raw-device
   ...
   from the duplicate copy, like other metadata, rustfs-spec §16);
...
5. preset round-trip per family: a synthetic ADFS file with `&FFF` filetype
   imported to RustFS stores `acorn.filetype = "fff"` plus exact load/exec, and
...
6. lossless or fail-closed: copying a RustFS file carrying `mac.type` to a mock
...
- **`AGENTS.md` §3** — add the shared `lib/fsmeta` crate and note the RustFS
  `attr`/metadata module if the charter enumerates RustFS internals.
...
- **`PLAN.md`** — add a RustFS extended-metadata stage (alongside/after the
  snapshot stage) and a one-line "Charter Amendments" rationale for any new
  crate or capability (charter §13).
- **`docs/src/filesystem/rustfs-spec.md`** — new §21, §2 feature-table row, §18

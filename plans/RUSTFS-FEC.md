...
changes, failure-safe recovery, and administration tooling to RustFS.
...
The current plan describes RustFS as copy-on-write, always encrypted,
...
- The current RustFS logical record size and maximum sealed-record size.
...
- If RustFS still uses `lib/compress`, use it.
...
: The smallest independently recoverable group of data and redundancy shards.
  A segment uses exactly one closed RustFS protection profile.
...
Add mandatory protection to RustFS regular file data so that:
...
- A silent reduction in protection to make an allocation, removal, or rebuild
  succeed.
- A no-FEC compatibility reader for old RustFS-native data.
- A `v2` beside `v1`, a compatibility shim, a migration-only old-format reader,
  or a feature flag preserving the old RustFS-native behaviour.
...
RustOS has not shipped. The RustFS-native format evolves in place. Obsolete
...
-> compression through the existing RustFS compression layer
...
- A RustFS bug that creates authenticated but semantically wrong plaintext
  before encryption.
...
Each RS data slot stores exactly one sealed RustFS data record or an explicit
...
```text
RustFS magic and current native format
pool UUID
device UUID
membership generation
placement epoch
...
or TUI code. Use the existing RustFS integrity abstraction and `lib/crypto`.
...
non-equivalent, following the existing RustFS security model exactly.
...
7. Mounts read-write only if the resulting state meets write-safety rules;
   otherwise mounts read-only when existing RustFS policy allows and reports the
   exact reason.
...
Before committing a relocated/reconstructed segment, RustFS evaluates the
...
domain, or sufficient safe distributed free capacity. RustFS must not pretend
...
RustFS must not keep accepting writes by silently using a lower profile.

### 20.6 Two-device limitation

After one device fails in a two-device mirror, no design can survive loss of the
remaining device. RustFS can still protect against local corruption on the
...
final record authentication under existing RustFS policy
...
RustFS exposes internal and administrator-visible accounting for:
...
read CPU time should regress by no more than 10% relative to the same RustFS
...
Use existing RustFS error types where semantically exact. Add a variant only
...
The mounted RustFS implementation exposes or is fronted by one versioned
...
What is RustFS doing, and can I safely pause it?
```
...
- Keep code private to RustFS unless a second production crate requires the math
...
- Wire the engine to a current RustFS formatter or segment-construction call site
...
- Field, matrix, shard ordering, and profile table in `rustfs-spec.md`.

### FEC2 - Single-device on-disk segment and shard format

Deliverables:

- Extend the current RustFS format in place with protected-segment descriptors,
...
- Fresh format creates local FEC-enabled RustFS only.
...
- Extend existing RustFS diagnostics without unnecessary ABI.
...
- All existing RustFS POSIX, fssoak, QEMU, crash, and fuzz suites remain green.
...
- Existing RustFS POSIX, fssoak, QEMU, crash-replay, corruption-injection, and
  fuzz suites.
...
- RustFS formats only protected native volumes.

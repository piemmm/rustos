...
ARXFS is already fully copy-on-write with a superblock ring, retained
transaction-root history, reflink/shared immutable chunks, refcounts, and a
reverse-reference tree (arxfs-spec §4, §9, §14). Those are exactly the
...
to a committed ARXFS root (a whole-volume point-in-time image of the root tree
...
- A snapshot is a **retained root** for the purposes of the existing safety
  ...
  snapshot root as live (arxfs-spec §11, §14). This is the rule already
...
- Snapshot metadata is held to the **same integrity, redundancy, encryption,
  and authentication** rules as all other authoritative metadata (arxfs-spec
...
- Snapshot operations are **crash-consistent**: a snapshot is created, renamed,
  ...
  the prior state or the new state, never a torn snapshot (arxfs-spec §14).
...
Snapshots build on the existing authoritative metadata (arxfs-spec §4). Two
...
implementation in `drivers/filesystem/arxfs/src/btree.rs`, charter §2.2 — no
...
SnapshotRecord:
    ...
    metadata-integrity/authentication fields inherited from ARXFS metadata
```
...
id, and lets `arxfs scrub`/`check` detect a snapshot root that no longer
...
secondary indexes", arxfs-spec §4) so lookup-by-name is O(log n) without
...
discard (arxfs-spec §5, §14). A snapshot **promotes** one of those committed
...
ARXFS already refcounts shared chunks and keeps a reverse-reference tree
(arxfs-spec §4, §9). Snapshots **must not** require bumping a per-block
...
- Block reuse and TRIM (arxfs-spec §11) compute "unreachable from every
...
All operations are one atomic COW transaction each (arxfs-spec §14).
...
- Every input validated: name length/bytes (arxfs-spec §13 name rules),
...
(ARXFS already stores a strong logical hash per record, arxfs-spec §2, §5).
...
DiffRecord:
    ...
    metadata    changed inode metadata + extended-attribute deltas (see
                plans/ARXFS-METADATA.md — backups must carry metadata too)
```
...
a `snapshot_receive` that reconstructs it into a target ARXFS volume,
...
- carries extended-attribute / preset metadata (plans/ARXFS-METADATA.md) so a
...
- **COW / transactions** (arxfs-spec §14): every snapshot op is one atomic
  transaction; crash leaves prior-or-new, never torn.
- **Reflink / dedupe / refcounts** (arxfs-spec §9): snapshots share the same
...
- **TRIM/discard** (arxfs-spec §11): the pending-discard queue gates on the
...
- **Scrub** (arxfs-spec §12): scrub verifies snapshot roots and the snapshot
  tree like any metadata; a snapshot root that fails to authenticate is a
  reported integrity event (charter §26.5).
- **Check** (arxfs-spec §12): offline check validates the snapshot tree,
  ...
  never make a volume unmountable (arxfs-spec §4).
- **Rescue** (arxfs-spec §12): rescue lists valid snapshot roots among the
  self-identifying metadata it discovers, so a damaged volume's snapshots are
  recoverable extraction targets.
- **Online grow** (arxfs-spec §13 resize): unaffected; snapshots pin roots, not
  device geometry.
...
- Tooling: extend the `arxfs` command (arxfs-spec §12) with
...
4. TRIM never touches a range reachable from any snapshot (extends the existing
   arxfs-spec §16 TRIM-reachability test to live snapshots);
...
10. `send`/`receive` round-trips a snapshot and an incremental diff, preserves
    ...
    preserves extended-attribute/preset metadata (ARXFS-METADATA.md), and
...
- **`AGENTS.md` §3** — the `drivers/filesystem/arxfs/` module list (arxfs-spec
  §3) gains a `snapshot` module; note it in the layout if the charter enumerates
  ARXFS internals there.

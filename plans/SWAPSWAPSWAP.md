...
- Every line of TAIRiX implementation is Rust, except the narrow assembly
...
TAIRiX shall provide an encrypted, compressed, RAM-resident tier for cold
...
`ramzip` must not wait until TAIRiX is one allocation away from OOM. At that
...
existing TAIRiX error vocabulary. Production paths must not panic.
...
- an existing approved TAIRiX compression abstraction, if present;
...
zstd-fast-style profile if TAIRiX already has such a layer.
...
On a compressed-page fault, TAIRiX restores the requested page first.
...
TAIRiX must not decompress the whole tier merely because pressure has gone
...
taskbar remain optional session frontends; headless TAIRiX must keep the same
...
- it must not rely on filesystem encryption alone unless the swap object is a
  first-class encrypted TAIRiX object with approved nonce and key handling;
...
- Compression through the approved TAIRiX compression abstraction.

...
- **P6d — userland process-spawn syscall `[~]`.** Add the `abi-v1`
  ...
  over the whole project on its own. **SP0 and SP1 (aarch64) are landed:**
  the SP0 design note is `docs/src/architecture/multitasking.md`, and the
  `kernel/core::kthread` runtime (`spawn_kthread`, the `Yielder`, the
  per-task kernel stack) is host-tested and proven on `-M virt` by
  `tests/integration/kthread_switch_qemu_aarch64` — two kthreads ping-pong
  through the real `ContextSwitch::switch`, now a production scheduling
  path. Remaining in SP1: the riscv64 + x86_64 sibling verticals.

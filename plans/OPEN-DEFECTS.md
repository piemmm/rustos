...
| D153 | services emit records at a rate an unprivileged client drives | noticed while adding `WINDOW_RETITLED` (`plans/NEW-DESKTOP-SETTINGS.md` DS13); not absorbed. A window opened and closed, a menu opened, or a title changed once per frame each costs one record through the session's `CAP_LOG_EMIT`, so a client holding no log authority can write the journal at frame rate; `netstack`'s `SOCKET_DENIED` and `discoveryd`'s `REQUEST_DENIED` are the same class, one record per refused call at the caller's call rate. Each record is honest and reaches only the diagnostic sink, never the kernel's audit log, and the bound belongs in the log path every service shares — a per-source record budget — rather than in each emitter |
...
| D200 | `shm_grant_peer` and `call_peer_holds` name their peer by its reusable pid | noticed merging the DMA engine; not absorbed. Both resolve the served call's poster through `peer_origin(ticket).pid()`, and `mint_grant_live` checks only that some process lives at that pid, not that it is the instance that posted the call, so a pid recycled between the lookup and the mint receives the grant. The fix compares the poster's `ProcId` under the lock the mint takes, which rests on the order process teardown releases a pid in |
| D201 | `call_peer_holds` answers for every resource kind | noticed merging the DMA engine; not absorbed. It reports whether the served caller holds a grant covering any quoted resource — a shared region, an IRQ, an endpoint — and does not check that a quoted `DmaRequest` names the endpoint being served; the kernel should admit only the kinds a DMA server asks about and require `line.endpoint() == endpoint` |
...
| D213 | the Settings bundle identifier is defined twice | noticed merging the desktop idle work; not absorbed. `userland/gui/session/src/config.rs` adds `SETTINGS_BUNDLE_ID`, equal by definition to `tairix_taskbar::system::SETTINGS_BUNDLE`, which the session already depends on |
...
| D215 | a test helper's rustdoc is split around another | noticed merging the desktop idle work; not absorbed. `userland/apps/settings/src/form.rs` inserts `activate_for_test` inside `choose_for_test`'s doc comment, so one carries both texts and the other none |

...
## D79 — half of every window's resize band showed a resize cursor and refused to drag (FIXED)

Reported as "I don't appear to be able to resize a window until I have moved
it". Two defects on the drag path; the first is the one a user hits.

- **The gesture was armed against the window rectangle, which excludes the
  outward half of every band by construction.** A band straddles the outer
  edge (`GrabReach`: `resize_edge_grab` 8 physical pixels at 100%, 4 out and 4
  in; `resize_corner_grab` 16, 8 and 8), and `Compositor::pointer_target`
  resolves a press out there as `PointerTarget::ResizeBand` — a variant that
  *only* arises for a point outside the window. `InputRouter::begin_resize_grab`
  then handed the shared `ResizeGrabber` `start_outer` as its hit region, so
  `on_pointer` found the press not `inside`, never set `dragging`, and every
  following motion answered `None`: `resize_drag_to` bailed with
  `FurniturePressed` and no edge ever moved. Escape was dead out there too —
  the cancel is the grabber's and its gesture had never begun. So the whole
  outward half of every band — the half a user aims at when the resize cursor
  appears over the backdrop beside a window — latched a grab, kept the double
  arrow, and did nothing, while the inward half worked. Fixed by giving the
  gesture the region it was resolved in: `WindowFrame::grab_region` (through
  `Compositor::window_grab_region`) is the outer rectangle grown by the widest
  band's outward reach on the three sides that carry one, and `ResizeGrab`
  captures it beside `start_outer` at grab start. `WindowFrame::hit` remains
  the gate deciding *which* edge a point grabs; the region only has to contain
  every point it accepts, which is why it is a bounding rectangle and not a
  second hit map.
  - Two tests already covered this ground and both stopped one assertion
    short: `the_overhang_announces_itself_and_a_press_there_resizes` (name
    included) and the session's `an_opened_window_is_pulled_fully_onto_the_work_area`
    each asserted the grab *latched* and never that a motion moved an edge.
    Both now drag.
- **A resize to the geometry already in force repainted the whole window.**
  `resize_to_outer` / `resize_client` return "the size was acceptable", which
  the drag needs as its window's liveness signal (a `false` from
  `resize_window` ends the grab), but `resize_window` treated it as "the
  geometry changed" and marked the old ∪ new bounds dirty and invalidated the
  window's rendered furniture. Most samples of a drag do not move the grabbed
  edge — the pointer wanders along the axis the edge does not follow, and a
  drag held at the minimum recomputes the same rectangle for as long as it is
  held — so a whole window's furniture was re-rendered and recomposited per
  sample for a geometry nobody could see change. Both paths now compare the
  bounds either side of the call and mark nothing when they match, which is
  the contract every sibling setter already keeps (`mutate`).
- **A drain sent one app-ward `Resized` per pointer sample where all but the
  last were duplicates by construction.** `fold_outcome` folded
  `ClientPointerMoved` and `AppScroll` but not `Resized`, and the embedder
  forwards a resize by reading the window's *current* client extent — after
  `pump` has applied the whole batch. Every sample of a run therefore carried
  the size the last one settled on. `pump` now keeps only the newest of an
  unbroken run over one window; `ResizeEnded` is the settle the app must
  witness and ends a run rather than joining it.

**Regression cover:** `lib/controls` (the grab region holds every point a band
claims, and a fixed-size frame offers none), `userland/gui/wm` (a drag from the
outward half of each of the five bands moves that edge by the pointer delta and
Escape restores exactly; a resize to the geometry in force marks no damage and
keeps the furniture, while a real one still repaints), and
`userland/gui/session` (every band of a freshly opened window drags, and a run
of resize samples folds to one outcome with the settle left standing).

**What this does not address.** The terminal's content still trails the frame
during a drag, and the remainder is a property of the protocol rather than a
defect this closes: `WindowRequest::Resize` carries a new shm handle, so the
app creates and maps a fresh frame region and repaints its whole grid each
time its size changes. It is already amortised to once per *cell step* rather
than per sample (`snap_to_cells` returns early when the snapped size is the one
in force), and a width change relayouts every row anyway, so a full repaint is
unavoidable there. What is left is the round-trip latency of the re-map and the
full repaint on a height-only change. Reducing either means a region sized to a
drag's high-water mark and an extent-only resize request — an
`plans/APPWIN.md` / `plans/GUI-TERMINAL.md` protocol decision, and one whose
gain should be measured on the target rather than assumed.

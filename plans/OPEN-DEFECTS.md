...
table normalises all three to **closed**. 38 open, 109 closed, 147 total.
...
| D149 | icon artwork landing repaints the whole icon bar and the whole library popup, where only the slots and rows that gained a picture changed | noticed while closing D148, and the same over-scoping class one tier up. `ArtworkDesk::take_landed` answers a bare `bool`, so `refresh_app_strip`'s `Taskbar::set_apps` and `DesktopShell::present_icon_artwork`'s `request_icon_repaint` both latch `Repaint::Whole` for the bar and the popup on every delivered batch — 48 672 px a batch on the `virt` board, measured at 5 across a boot and 1 inside the hover bracket once D148's per-frame latch was gone, against a bracket total of 520 713 – 788 713 px. Bounded and small, so it is recorded rather than absorbed: the surfaces already *store* a picture per slot and per row, so the honest fix is the pattern `resolve_library_icons` uses on the read side — the desk reports which jobs landed, and each surface latches only the items whose stored picture the batch replaced. That is a change to the desk's public answer, to `IconRequest`'s tier keys, and to both surfaces' adopt paths, which is its own body of work |
| D150 | the boot stack has no overrun detector on any port: an overrun corrupts the `.bss` statics below it and is diagnosed only by what breaks next | this is the control the `figure-determinism-qemu-riscv64` failure was missing. The figure reference grid needs more than the 64 KiB `boot.s` then reserved, so the boot hart ran off the bottom of its stack into `.bss`, clobbered the `CONSOLE` frame header, and `tairix_conout`'s `peek` underflowed `length - head_sent` — a panic on the console path, reported *through* that console, so the machine went silent for the whole 90 s budget instead of naming a cause. `c9d19b4a7` raised the size (256 K on `riscv64-virt`/`aarch64-virt`, 64 K on `aarch64-rpi4`) and placed the stack last in `.bss` so an overrun grows into zeroed statics rather than `.rodata`/`.text`. That is sizing and blast-radius containment, not detection: the next workload to outgrow its stack corrupts `.bss` silently again. The MMU is off on the boot hart, so no page can be unmapped below the stack and the defence has to be a poisoned guard — the linker reserving a page below `__boot_stack_bottom`, filled with the shared sentinel, its top `CANARY_BYTES` verified on the panic path so a corrupted-`.bss` panic names the overrun. The shared vocabulary for that now exists (`lib/memguard`), and the kthread guard is the working precedent (`kernel/core`'s `BoxStack`, checked in `dispatch_step`). **The open design question is how Rust writes a linker-reserved region soundly**: `extern "C" { static … }` is immutable, so filling it through `addr_of!(…).cast_mut()` is UB by the strict reading, while `static mut` is banned outright as a hack, and hoisting the extern block into a shared crate cannot work because `riscv64`/`aarch64` share the symbol names but `x86_64` does not and a host build defines none. The candidates are a `Sync` newtype over `UnsafeCell` for the extern static, exposed-provenance (`with_exposed_provenance_mut`), or doing the fill in `boot.s` beside the existing `.bss` zero loop and letting Rust only *read* the guard — the last is sound and needs no unsafe write, but re-spells the sentinel in assembly, which is the duplication the charter forbids unless the linker script carries the one definition. Pick one before writing the ports; landing it on one architecture while its twin re-derives it later is itself a defect. Related: D146, which is why the fault presented as silence rather than a report |

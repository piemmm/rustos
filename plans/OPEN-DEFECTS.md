...
table normalises all three to **closed**. 34 open, 107 closed, 141 total.
...
| D144 | `menu-qemu-aarch64` stalled once at its runtime ceiling with the terminal never launched, and the mechanism is not known | observed once in a full 182-test matrix run on `6f1895cc5`; did not reproduce in three later runs (standalone 22.4 s, and two full matrices at 25.1 s and green). **Not** load: the guest was alive and idle at the kill (≈8 IPC/s, silent 1.98 s) and 600 s dwarfs the 22 s a pass needs, so it stalled rather than ran slow. Reached `desktop fully revealed` + `first input delivered kind=pointer`, then nothing: no `terminal.app` bundle load and so no `served window first frame on screen`, which is the gate the rest of the script waits on. The launch click therefore had no effect. Suspicion, unconfirmed: `menu_pointer_script` gates *both* the library-button and entry-row clicks on the same `AUTOLOAD_DESKTOP_REVEALED_MARKER`, so nothing proves the program-library popup is on screen before the row click is sent, and a passing run completes the whole launch inside 0.66 s of that marker — a thin margin. No session marker exists for "library popup on screen" (`MENU_SHOWN` is the window menu). Diagnosing it needs the failing serial log kept: the reproduction attempt overwrote it, since `persist_serial` rewrites one path per test |

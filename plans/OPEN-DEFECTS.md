...
**closed**, and a partial fix stays **open**. 101 open, 220 closed, 321 total.
...
| D209 | the ports' own fatal reports can be re-entered without bound | noticed merging the fault-path rework; not absorbed. `handle_panic_via_serial` and `report_unclaimed_fault` have no latch like the kernel core's `PANICKING`, so a panic whose payload's `Display` panics recurses until the stack overruns — on x86_64 into `boot_pds`, the silent triple fault D146 was. One latch in `tairix_arch_api::fatal::Reporter` writing the bare record on re-entry is the fix |
| D210 | the fault reports disagree across ports in what they read and when | noticed merging the fault-path rework; not absorbed. aarch64's prologue takes the console lock, which uses exclusive loads and stores, while the MMU may be off; `cpu=` is the dense id on aarch64, the SBI hart id on riscv64 and either on x86_64; x86_64 judges an IST delivery from the per-CPU mapping even under the boot tables and drops the interrupted `rsp` its stub passes; and `refuse()` ends a production boot through the QEMU debug-exit port |
| D211 | the fatal-fault verticals repeat the harness's build glue | noticed merging the fault-path rework; not absorbed. Their `build.rs` files repeat the linker-script stanza, one of 53 copies in the tree (the x86_64 fatal-fault pair now share `x86_64_guest_build`), and the boot-stack guard check is written in three ports and the kernel core |
| D212 | the x86_64 boot tables' contents are untested | noticed merging the fault-path rework; not absorbed. `percpu.rs` tests `boot_ist_for_vector` alone, never the table `fatal_table` builds; the fatal-fault verticals are the only witness |
...
| D313 | `lib/util::mathf`'s `sin`, `cos` and `tan` take every angle past 2^20 quarter turns (about 1.65e6 rad) at that bound, so they answer a constant beyond it | noticed reviewing the merge of `ac3ecbb5d`; not absorbed. `reduce` clamps to `REDUCIBLE` where the code before it wrapped by whole turns: cinder's never-wrapped `gait::tail_sway` (`sin(elapsed * 1.7)`) stops moving after about eleven days of uptime, and an SVG `rotate`/`skew` past about 9.4e7 degrees collapses to none. The fix is full-range reduction (Payne–Hanek), or at the least the whole-turn wrap beyond the bound, with a test past it |
| D314 | `lib/window::app` remembers a refused present's damage as `torn` and widens every later present by it, so one damage rectangle the frame codec refuses makes every present after it fail until a resize or a content release | noticed reviewing the merge of `ac3ecbb5d`; not absorbed. A rectangle running past the surface is refused by `Shape::resolve`, and so is every widened one after it; cinder ignores present errors, so its window would freeze silently. The fix clips `torn` to the surface, or records nothing for a codec refusal. The module builds only for the bare-metal targets, so `try_present` and the re-send have no host test at all |
| D315 | the QEMU harness can send a typed key or a pointer step before the screendump gated on the same marker is pending | noticed reviewing the merge of `ac3ecbb5d`; not absorbed. The serial reader sets a marker's input flags before its dump flag, and the input loop reads them unlocked (a key reads its own flag first, a pointer step the dump's), so a descheduled reader lets input overtake the dump; the WinterSun client vertical's `f11`/`esc` keys and toggle/close clicks rely on the order, and `a_typed_key_waits_for_a_pending_dump` sets both flags together so never tests it. The fix publishes dump flags before input flags and reads input before dumps, or reads all of them under one lock |
| D316 | screendump paths are per enrolment, not per replica, so the flake hunt's concurrent replicas delete and overwrite each other's dumps | noticed reviewing the merge of `ac3ecbb5d`; not absorbed. Dumps are `kernel.with_extension("<suffix>.screendump.ppm")` where `sidecar_path` is per replica: one replica's stale-file cleanup removes another's, its QEMU overwrites them, and a runner can check a sibling's pixels (false failures and false passes). `sibling_screendump` hard-codes the scheme, and `sidecar_paths_never_collide_across_enrolments_or_replicas` covers only `arxfs.img` and `serial.log` |
...
| D319 | WinterSun's pinned digest lost its not-blank check on its own two shots | noticed reviewing the merge of `ac3ecbb5d`; not absorbed. `digest_tests::the_frames_are_not_blank` gave way to `reference_tests`' check of the window shot alone (default zoom, full detail), in the change that re-pinned the digest |
| D320 | the session asks every shown window whether it is visible on every presented frame | noticed reviewing the merge of `ac3ecbb5d`; not absorbed. `windows.rs`'s size-change check calls `visible()`, a linear `on_display` search, for each window per frame, O(n^2), where it used to run only with a retitle pending; testing `record.sized` first avoids it |
| D321 | `lib/util::mathf`'s `exp` fails a little short of the range its bounds document | noticed reviewing the merge of `ac3ecbb5d`; not absorbed, and older than it. `exp(709.5)` answers `f64::MAX` rather than 1.35e308 and `exp(-708)` answers 0 rather than 3.3e-308, a normal double, so `EXP_MAX_ARG` and `EXP_MIN_ARG` state a range the function does not reach |
| D322 | docs the move of `event_endpoint_for` and the re-send left stale | noticed reviewing the merge of `ac3ecbb5d`; not absorbed. `lib/window/README.md` and `userland/apps/terminal/Cargo.toml` still place `event_endpoint_for` in `lib/window`, the README describes neither `try_present` nor the re-send, `Compositor::has_damage` no longer answers whether the next `composite()` changes pixels, and in `qemu_tests.rs` the WinterSun client test sits between the Settings test's doc comment and its function |
...
| D324 | `tools/syshelp` plants a bundle directory named from an unchecked manifest `name`, and the harness checks named keys only when a step reaches them | noticed reviewing the merge of `ac3ecbb5d`; not absorbed, and defence in depth only. `build.rs` takes `name` through `quoted_string`, which admits `/` and `..`, leaving the plain-command-word check to the harness's discovery; `TypedKeys::Named` names are validated after a multi-minute boot rather than in `with_named_keys` |

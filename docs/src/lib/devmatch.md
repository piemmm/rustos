...
RustOS (`AGENTS.md` §18.3). A discovered hardware-tree node
(`rustos_abi::HwNode`) carries the match keys its discoverer emitted; a driver
...
It depends only on `rustos-abi` (the wire types it compares).

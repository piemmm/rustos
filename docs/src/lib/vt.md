...
| `secret` | The secret-entry activity indicator: `SecretIndicator` / `SecretInput` — the `[input active...]` marker every echo-suppressed (password) prompt shows after the first typed character, its dots cycling `.` → `..` → `...` on a one-second cadence for as long as the marker is shown, and the whole marker removed on Enter or when the input is erased back to empty. A pure, clock-free state machine emitting plain text plus `BS SP BS` rub-outs; the kernel console hosts and renders it with one-shot deadlines (tickless — nothing is armed until the first typed character). |

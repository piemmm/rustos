...
| *(crate root)* | `CONVENTIONAL_COLUMNS` × `CONVENTIONAL_ROWS` — the 80×25 character-cell screen every command-line program lays itself out for, defined once (`AGENTS.md` §2.2). It is the floor `lib/fbcon` keeps when it sizes the text grid to the discovered panel (magnifying the cell by the largest whole factor that still leaves it) and the grid `terminal.app` opens at, so the console and the terminal emulator cannot disagree about what a normal screen is. A floor, never a cap: a surface with room for more columns shows more. |

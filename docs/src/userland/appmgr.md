...
lives in the shared [`rustos-appload`](../lib/appload.md) crate, so the one
...
see the [`rustos-appload` page](../lib/appload.md).

The crate is `no_std` and depends only on `rustos-appload` (itself `lib/*`),

...
| `16_019` | `BOND_FAILOVER` | Info | A bond's transmit path changed member **while it was already transmitting** (failover or deliberate failback); the bond re-announced its presence so peers relearn the path — a dead member is a visible, audited fact. Recorded whether or not an announcement went with it, so a bond with nothing announceable (no address yet, or one still in DAD) still surfaces. |

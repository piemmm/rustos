...
aarch64 `ConfiguredPhysMap`, not the arch-neutral `DirectPhysMap`
...
`ConfiguredPhysMap`; both reach the frame through the same direct map,
only the maintenance differs.

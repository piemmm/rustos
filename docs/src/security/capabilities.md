...
`elevate <user> <program> [argument ...]` builtin
(`plans/CAPABILITY_USE.md` CU5): it posts one synchronous IPC call to its
console's login supervisor over the reserved per-console rendezvous
(`lib/abi/src/elevate.rs`, derived from the caller's kernel-attested
`Origin::console`), the supervisor re-authenticates the offered
credentials with the same timing-equalised authenticator as the login
prompt and spawns the program as the target account with the request's
bounded argument vector, and the shell blocks until it exits. The vector
is what lets a caller run the tool that already owns a store with the one
change a user asked for rather than a second writer for that store
existing; it widens no authority, because the request already named an
arbitrary absolute program. The elevated child's set is

# Calling TAIRiX from C (a worked example)
...
TAIRiX itself stays Rust-only (`AGENTS.md` §1). Nothing here adds C to the OS;
it shows how a *hosted* third-party program in any C-FFI language reaches the
TAIRiX kernel, and how the in-tree CC5 integration test proves the header, the
syscall stub runtime, and crt0 all agree with the Rust side.

## The three pieces a C program links

A non-Rust program needs exactly three things from TAIRiX, all already on the
system:

1. **The headers** under `include/tairix/` — the C view of every `abi-v1`
   type, constant, and `tairix_sys_<name>` prototype. They are generated from
   `lib/abi` (`cargo xtask c-header --write`) and never hand-edited.
2. **The syscall stub runtime** (`lib/abi-sys`) — one `extern "C"`,
   export-name-pinned `tairix_sys_<name>` per syscall that marshals registers and
...
3. **The startup object, crt0** (`lib/crt0`) — provides `_start`, marshals the
   ...
   its return through `tairix_sys_exit`.
...
```c
#include <stdint.h>
#include "tairix/tairix_capability.h"
#include "tairix/tairix_syscall.h"
#include "tairix/tairix_time.h"
#include "tairix/tairix_ipc.h"
#include "tairix/tairix_sysinfo.h"

int main(int argc, char **argv, char **envp) {
    (void)argc; (void)argv; (void)envp;

    /* A Time64 spans the §21 pre-1970 / post-2038 boundaries in 64-bit secs. */
    tairix_time64_t pre  = { .secs = -2208988800LL, .nanos = 500000000u };
    tairix_time64_t post = { .secs =  4102444800LL, .nanos = 1u };
    if (pre.secs >= 0 || post.secs <= (int64_t)INT32_MAX) {
        return 81;
    }

    /* abi-v1 wire types are usable directly from C. */
    tairix_ipc_message_header_t msg = { .magic = TAIRIX_IPC_MESSAGE_HEADER_MAGIC };
    tairix_sysinfo_request_header_t req = { .magic = TAIRIX_SYSINFO_REQUEST_MAGIC };
    ...
    if (tairix_sys_cap_query(TAIRIX_CAP_DRV_KERNEL) != 1u) {
        return 84;
    }
    return (tairix_sys_clock_get() != 0u) ? 0 : 85;
}
```
...
TAIRiX compiles the C program with `clang` and links it with `ld.lld`, but
never as an unaudited shell-out (`AGENTS.md` §12). The single gateway is the
`tairix-cc` crate (`tools/cc`), which:
...
- resolves `clang` / `ld.lld` automatically — no configuration once the pinned
  ...
  (`TAIRIX_CC_CLANG` / `TAIRIX_CC_LLD`, authoritative), the versioned name on
...
- runs `--version` and fails closed unless the tool reports the pinned
  `tairix_cc::REQUIRED_CLANG_VERSION` / `REQUIRED_LLD_VERSION`,
- SHA-256-hashes each binary with the audited `lib/crypto` and records it for
  the build transcript (and verifies it against `TAIRIX_CC_CLANG_SHA256` /
  `TAIRIX_CC_LLD_SHA256` when a digest is pinned).
...
cc.link(&LinkRequest {                            // ld.lld -> PIE ELF
    target: CTarget::Riscv64,
    objects: &[&object],
    archives: &[&runtime_archive],               // crt0 + tairix_sys_* runtime
...
`tairix_abi::rxe::LoadImage::parse`, on a C binary exactly as on a Rust one:
...
targets**: `tairix-test-c-program-qemu-riscv64` (riscv64 `virt`, U-mode),
`tairix-test-c-program-qemu-aarch64` (aarch64 `virt`, EL0), and
`tairix-test-c-program-qemu-x86_64` (x86_64, ring 3). Each build script builds
the crt0 + `tairix_sys_*` runtime shim (`tests/integration/cc5_program`) as a
position-independent `staticlib`, compiles `main.c` with `tairix-cc`, links one
...
`tairix_kernel_core::spawn_and_enter` and installs a dispatch callback that
...
§4). The `tairix_sys_*` stubs only marshal registers and trap; the kernel

...
//! (`AGENTS.md` §16.7): one **controller** process — pinned via `mem_pin`,
//! opted into `signal_intake` so `^C` tears the run down cleanly — spawns
//! N swappable **worker** children (the same binary re-entered in worker
//! mode through the kernel's attested `@self` spawn token), one per
//! requested load unit, that load the CPU, memory, filesystem write path,
//! disk throughput, and the kernel caches, separately or combined, with a
//! configurable overcommit level, a run timeout, quiet and background
//! modes, and an option to run `sysmon` alongside.
//!
//! # What this crate is
//!
//! The fully host-testable substance of the tool:
//!
//! * [`command`] — the closed §7.3 option grammar and its fail-closed
//!   [`parse`]r.
...
//! `unwrap`/`expect`/`panic!` in production paths. Every figure the
//! sizing reads travels through the System Information API (§16.6); the
//! workers hold no authority beyond the caller's own.

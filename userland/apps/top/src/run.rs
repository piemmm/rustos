...
//! program's `main`.
//!
//! `main` sizes the screen from the console the kernel gave it, puts the
//! terminal into raw (no-echo) input, and runs the [`rustos_top`] viewer loop
...
mod program {
    ...
    ///
    /// Exit codes: `0` on a clean quit, `1` on a service or terminal failure.
    fn main() -> i32 {

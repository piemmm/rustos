...
그립니다). 시간 기록 열은 기본적으로 수정 시간을 보입니다. `-c`, `-u`, `--time`은 네 가지 시간 기록 중 어느 것을 표시(및 정렬)에 쓸지 고르며, `--time-style`(또는 `--full-time`)이 그 형식을 정합니다. 링크 수 열은 그 파일을 가리키는 디렉터리 항목이 몇 개인지를, 파일 시스템이 기록한 그대로 보입니다. 수를 두지 않는 형식은 `1`이라고 답합니다.

피연산자가 여럿 주어지면 — 그리고 `-R`에서는 언제나 — 각 디렉터리의 목록
앞에 `path:` 머리글이 오고, 블록들은 빈 줄로 구분됩니다.

심볼릭 링크는 형식 문자 `l`로 표시되고, 긴 형식에서는 `이름 -> 대상`으로
나타난다. 대상은 저장된 그대로이며 해석하지 않는다. 그것이 링크가 담고 있는
내용이기 때문이다. 그러므로 매달린 링크도 평소처럼 나열된다. 대상에 이를 수
없다고 알리는 것은 그것을 해석하는 자세(`-L`, 또는 피연산자에 대한 `-H`)뿐이다.
...
- `-L, --dereference` — 심볼릭 링크가 어디에 나타나든 링크 자신이 아니라 그것이
  가리키는 파일의 정보를 보인다. 대상에 이를 수 없는 링크는 표준 오류에 알리고
  나열은 계속되며 종료 상태는 0이 아니다.
- `-H, --dereference-command-line` — 명령줄에 적힌 심볼릭 링크만 따라간다. 나열
  안쪽의 링크는 링크로 보인다. `-L`과 `-H` 중 나중 것이 이긴다.
- `--dereference-command-line-symlink-to-dir` — 어떤 형식 옵션도 달리 강제하지
  않을 때의 기본값. 명령줄의 *디렉터리를 가리키는* 링크는 따라가므로
  `ls linkdir`는 그 디렉터리를 나열하고, 그 밖의 링크는 자신을 보인다. `-l`,
  `-d`, `-F`는 반대로 모든 링크를 자신으로 보인다.

...
//! [`rustos_ps`] grammar, and runs the resulting command against the two
...
//! rendered row to the inherited standard output (fd 1). The tool binds only
//! to its inherited descriptors, never a console device, and holds no
//! ambient authority: `sysinfod` gates every query against the caller's

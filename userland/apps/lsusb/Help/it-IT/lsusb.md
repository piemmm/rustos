...
Elenca, una riga per ogni interfaccia USB rilevata, i numeri di bus e
di dispositivo dell'interfaccia, il suo identificatore
...
RustOS non ha il registro Linux dei numeri di bus/dispositivo: il
numero di bus di un dispositivo è l'identificatore di nodo stabile del
suo controller nell'albero hardware e il suo numero di dispositivo è
il proprio identificatore di nodo, e `-s` seleziona quegli
identificatori (una divergenza deliberata e documentata rispetto a
`lsusb` di Linux). L'inventario registra un nodo per *interfaccia*: un
dispositivo con più interfacce compare una volta per interfaccia.

## OPTIONS

- `-v` — dopo ogni dispositivo, elencare la classe, la sottoclasse e
  il protocollo dell'interfaccia (`bInterfaceClass`,
  `bInterfaceSubClass`, `bInterfaceProtocol`) con i nomi delle tabelle
  delle classi USB.
- `-t` — mostrare i dispositivi come un albero sotto i loro controller
  e bus.
...
- `-s [[<bus>]:][<devnum>]` — elencare solo i dispositivi che
  corrispondono agli identificatori di nodo del controller (bus) e/o
  del dispositivo (decimale); un valore senza due punti è un numero di
  dispositivo da solo.
...
- `lsusb -v` — lo stesso, con l'identità di classe di ogni
  interfaccia.
- `lsusb -s 2:` — ogni dispositivo sotto il nodo controller 2.

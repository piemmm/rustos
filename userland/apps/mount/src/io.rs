...
//! and [`Output`](rustos_procinfo::Output) seams from `lib/procinfo`
//! (`AGENTS.md` §16.6); only the privileged *attach* operation needs a seam
...
pub struct MountSpec<'a> {
    ...
    /// The mount-policy flags to apply (`AGENTS.md` §5.3).
    pub flags: MountFlags,
}

/// Attaches a filesystem to the mount table.
///
/// Mounting is privileged — it needs `CAP_FS_MOUNT` (`AGENTS.md` §5.2) — but
/// the **kernel** makes that decision, not this tool (`AGENTS.md` §5.4):

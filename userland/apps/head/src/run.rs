//! The `Run` entry-point binary of the `head` tool — the program a shell
//! spawns to output the first part of files.
//!
...
//! `RtOutput`/`RtErrors`, which write to the inherited standard output and
...
mod program {
    ...
    use rustos_head::{parse, run, FileSource, Input, Output, USAGE};
    ...
    fn main() -> i32 {
        ...
        let command = match parse(&arguments) {
            Ok(command) => command,
            Err(err) => {
                write_stderr_line(&format!("head: {err}"));
                write_stderr_line(USAGE);
                return 2;
            }
        };

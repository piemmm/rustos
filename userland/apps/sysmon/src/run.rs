//! The `Run` entry-point binary of the `sysmon` tool — the program a shell
...
//! line beyond `-d` is a usage error — the monitor's controls are keys
//! pressed inside the session). It then **pins its own memory**
//! (`mem_pin`): the monitor must never stall on its own page fault-in under
//! the very pressure it observes. A refused pin — no `CAP_MEM_PIN` in the
...
//! and holds no ambient authority: `sysinfod` gates every query against the
//! caller's kernel-attested origin.
//!
//! # Terminal size
//!
//! The monitor draws into a fixed grid, so it asks the kernel how big its
...
mod program {
    ...
    ///
    /// The pin is incidental to the monitor's purpose: a refusal is
    ...
    fn pin_self() -> PinState {
        ...
        if errno == Errno::PermissionDenied.as_i32() {
            PinState::Unpinned("CAP_MEM_PIN not held")
        } else if errno == Errno::OutOfRange.as_i32() {
            PinState::Unpinned("pinned-memory limit exceeded")
        } else {
            PinState::Unpinned("pin refused")
        }
    }
    ...
    fn main() -> i32 {
        ...
        // Pin before the first refresh, so the monitor's pages are exempt
        // from the swap tiers for the whole session it observes them.
        let pin = pin_self();
        ...
        // The raw input discipline: keystrokes reach the monitor verbatim
        ...
        // exit), an in-place erase otherwise — either way the monitor never
        // draws over stale text from the previous command.
        let entered = screen.enter_full_screen();
        let transport = IpcTransport;
        let mut model = Model::new(delay_tenths);
        model.set_pin_state(pin);
        ...
        if let Err(err) = &result {
            write_stderr_line(&alloc::format!("sysmon: {err}"));
        } else if entered.is_err() || left.is_err() {
            write_stderr_line("sysmon: terminal error: the screen could not be switched");
        }

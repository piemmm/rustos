//! The `Run` entry-point binary of the `userdel` tool — the program an
//! administrator's shell spawns to delete an account.
//!
...
mod program {
    ...
    use tairix_userdel::{parse, run, Output, USAGE};
    ...
    fn main() -> i32 {
        ...
        match run(
            ...
            locale,
            &RtChannel,
            &BundleHelp::new("userdel"),
            &RtOutput,
        ) {
            Ok(()) => 0,
            Err(err) => {
                write_stderr_line(&format!("userdel: {err}"));
                1
            }
        }
    }

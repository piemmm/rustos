...
use crate::appearance::{Form, FormPlace};
...
impl Body {
    ...
    pub(crate) fn of(pane: &PaneRow, answered: &Answered<'_>) -> Self {
        match pane.content() {
            None => Self::Statement,
            Some(PaneContent::Form(composition)) => Self::Form(Form::new(
                composition,
                answered.settings,
                answered.cursor_sets,
            )),
            Some(PaneContent::Pictures(composition)) => Self::Pictures {
                form: Form::new(composition, answered.settings, answered.cursor_sets),
    ...
    pub(crate) const fn form(&self) -> Option<&Form> {
        match self {
            ...
            Self::Statement | Self::Volumes(_) => None,
        }
    }
    ...
    pub(crate) const fn gallery(&self) -> Option<&Gallery> {
        match self {
            ...
            Self::Statement | Self::Form(_) | Self::Volumes(_) => None,
        }
    }
    ...
    pub(crate) fn adopt(&mut self, settings: &DesktopSettings) {
        match self {
            ...
            // Neither reads the desktop's document: one states the
            // registry's own words, the other the machine's volumes.
            Self::Statement | Self::Volumes(_) => {}
        }
    }

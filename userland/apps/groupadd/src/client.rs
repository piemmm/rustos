...
/// short-help switches print when `groupadd`'s own Help tree is unavailable.
pub const USAGE: &str = "\
...
/// `groupadd`'s own command word: the short-help switches render its own
/// Help document through the same engine as any other command's.
const OWN_WORD: &str = "groupadd";

/// Run one [`Command`], creating the group through `db`. `locale` is the
...
/// Render `groupadd`'s own short help (`NAME` + `SYNOPSIS` + compact
...
fn short_help(
    ...
    out.write_all(&bytes).map_err(GroupaddError::Output)
}
...
mod tests {
    ...
    /// A Help tree holding one canonical `groupadd.md` document.
    struct OneDoc;
    ...
    impl HelpSource for OneDoc {
        ...
        fn read(&self, locale_dir: &str, file_name: &str) -> Result<Option<Vec<u8>>, SourceError> {
            if locale_dir == "default" && file_name == "groupadd.md" {
    ...
    fn help_renders_the_own_document_when_served() {
        ...
        assert!(text.contains("groupadd"));
        assert!(text.contains("create a group"));
        assert!(
            !text.contains("Creates a group."),

...
//! without a screen: the elevated camera ([`project`]), the soft fur splat
//! ([`fur`]), the body ([`cinder`]), locomotion ([`gait`]), what he decides
//! to do ([`mind`]), the desktop he walks ([`world`]), how he gets about it
//! ([`roam`]), the pen ([`pen`]), its geometry ([`layout`]), the two painters
//! ([`paint`]), and what survives a restart ([`state`]).
#![no_std]
#![forbid(unsafe_code)]
#![deny(missing_docs)]

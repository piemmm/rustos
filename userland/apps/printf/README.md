...
- GNU `printf` computes floating-point conversions in C `long double`;
  RustOS computes in IEEE 754 `f64` (`double`), through the same shared
  C-locale renderer as `seq -f` (`rustos_util::cfloat`). A value beyond
...
- `-h`/`-?`/`--help` as the first argument serve the RustOS short-help
...
- A numeric character constant (`'x`) converts the argument's first
  *character* (its Unicode scalar value — RustOS argument vectors are
...
`rustos_util::cnum`) behind injected `Output` seams, plus the freestanding

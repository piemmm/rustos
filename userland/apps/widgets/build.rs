...
//! bare-metal target (`target_os = "none"`), so the widget-gallery `Run`
//! binary (`src/run.rs`) compiles as a freestanding pure-Rust program there
//! and as an inert host stub everywhere else (`plans/APPWIN.md` AW3).
//!
...
//! xtask cfg-check` stays clean. It mirrors `userland/apps/files/build.rs`.

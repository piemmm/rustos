...
/// Why a `groupdel` run did not complete.
...
pub fn run(
    ...
    match command {
        Command::Help => {
            let text = tairix_help::own_short_help(help, locale, "groupdel")
                .unwrap_or_else(|| crate::USAGE.as_bytes().to_vec());
            out.write_all(&text).map_err(RunError::Output)
        }
        Command::Delete(name) => {
            let name = name.as_str();
            submit(channel, &UsersAdminRequest::DeleteGroup { name }).map_err(RunError::Refused)
        }
    }
}

...
//! arguments with the pure [`rustos_configure`] grammar, and runs the
//! resulting command against the production seams: the syscall-backed store
//! file at `rustos_sysconfig::CONFIG_PATH` (read and replaced whole through
...
//! `rustos_help::BundleHelp` for the short-help switches, and the inherited
...
mod program {
    ...
    use rustos_configure::{parse, run, ConfigureError, Store, USAGE};
    use rustos_help::BundleHelp;
    use rustos_rt::io::{write_stderr_line, Stdout, Write};
    use rustos_sysconfig::{CONFIG_DIR, CONFIG_PATH, MAX_CONFIG_LEN};
    ...
    impl FileStore {
        ...
        fn read_all(fd: u32) -> Result<String, Errno> {
            ...
            while bytes.len() <= MAX_CONFIG_LEN {
                let read = rustos_rt::fs_read(fd, bytes.len() as u64, &mut chunk)
    ...
    impl Store for FileStore {
        fn read(&self) -> Result<Option<String>, Errno> {
            let ret = rustos_rt::fs_open(CONFIG_PATH.as_bytes(), OpenFlags::READ);
            if ret < 0 {
        ...
        fn write(&self, text: &str) -> Result<(), Errno> {
            ...
            let ret = rustos_rt::fs_mkdir(CONFIG_DIR.as_bytes());
            ...
            let ret = rustos_rt::fs_open(CONFIG_PATH.as_bytes(), flags);
            if ret < 0 {
                return Err(Errno::from_syscall(ret));
            }
            #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    ...
    fn write_all(fd: u32, bytes: &[u8]) -> Result<(), Errno> {
        ...
        while written < bytes.len() {
            let n = rustos_rt::fs_write(fd, written as u64, &bytes[written..])
                .map_err(Errno::from_syscall)?;
            if n == 0 {
    ...
    /// The production [`rustos_configure::Output`] over the inherited
    /// standard output (fd 1).
    struct RtOutput;

    impl rustos_configure::Output for RtOutput {

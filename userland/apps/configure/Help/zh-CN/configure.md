...
- `net.sockets.mem` — `auto` 或形如 `64M` 的字节大小：网络栈在所有主体
  之间可用于套接字状态的内存。`auto`（默认）依据本机内存推导，使大型服务器
  不受限于在小机器上选定的数字；给出大小则由更了解负载的管理者覆盖它。每个
  主体可持有实际预算的十六分之一。以字节而非套接字数量计，因为同样数量的
  套接字在空闲时只有几千字节，缓冲满时则达数兆字节：预算按负载的真实形态，
  或承载许多安静连接，或承载较少的繁忙连接。

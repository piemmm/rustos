...
//! lexical directory-name surgery ([`rustos_dirname`] — no operand path is
...
mod program {
    ...
    use rustos_dirname::{output, parse, Command, USAGE};
    use rustos_help::{own_short_help, BundleHelp};
    use rustos_rt::io::{write_stderr_line, Stdout, Write};

    /// Render `dirname`'s own short help (`NAME` + `SYNOPSIS` + compact
    ...
    fn short_help() -> i32 {
        ...
        let bytes = own_short_help(&BundleHelp::new("dirname"), locale, "dirname")

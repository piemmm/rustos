...
- `show <resource-ref>` — `info:`/`state:`/`stats:` 리소스 참조 하나를 읽어
  값을 출력합니다. 이 네임스페이스들은 이 API 를 통해 형식이 있는 값을
  제공하며 바이트 스트림이 아니므로 `cat` 으로 열 수 없습니다. 거부되면
  필요한 케이퍼빌리티를 알려 줍니다.
- `describe <resource-ref>` — 값 대신 응답 봉투를 출력합니다: 생산자, 제공에
  사용된 권한, 그리고 페이로드 자체의 메타데이터 — 메트릭이면 종류·단위·재설정
  동작·샘플링 창, 사실이면 타입과 민감도.

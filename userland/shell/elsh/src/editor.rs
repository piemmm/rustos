...
impl<'a> Session<'a> {
    ...
    ///
    /// Every candidate is real text the shell is willing to insert: the
    /// engine offers a resource-selector placeholder as the names behind it
    /// or not at all, so there is no display-only class the editor must
    /// refuse to insert.

//! TAIRiX seat-manager service (`seatmgr`) — `plans/DISPLAY.md` D3.
//!
...
//! [`tairix_abi::seat::SEATMGR_ENDPOINT`] rendezvous and serves the typed
//! [`tairix_abi::seat::SeatAdminRequest`] operations over the kernel's
...
//!
//! 1. Decode the [`SeatAdminRequest`](tairix_abi::seat::SeatAdminRequest),
...
//! 3. Emit a [`tairix_log`] audit record for every applied operation,
...
//! # Module map
//!
//! * [`events`] — stable [`tairix_log::EventId`] constants (`14000` range).
...
//! `tairix-abi` and `tairix-log`, so a userland service never links a

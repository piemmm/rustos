# `rustos-appmgr` — application-manager service
...
shared `lib/appload` crate (`rustos-appload`), so the one gate is used by both
...
surface, see `lib/appload` (`rustos-appload`).

## Layering & safety

`no_std`, depends only on `rustos-appload` (itself `lib/*`), so the userland

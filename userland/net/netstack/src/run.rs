...
mod program {
    use rustos_abi::driver::net::ETHERNET_HEADER_LEN;
    ...
    impl NetChannelTransport for RtNetChannelTransport {
        fn call(&mut self, request: &[u8], reply: &mut [u8]) -> Result<usize, Errno> {
            rustos_rt::ipc_call(self.endpoint, request, reply).map_err(errno_from)
        }
    }
    ...
    fn serve_admin(
            ...
            NetstackRequest::from_bytes(&request[..request_len])
        {
            ...
            let _ = rustos_rt::call_reply(NETSTACK_ENDPOINT, ticket, &encode_status_reply(result));
            return;
        }
        match serve(
            stack,
    ...
    fn bind_driver(
        ...
        let base = rustos_rt::shm_create(region_len, &mut region_id);
        if base < 0 {
        ...
        let grant = rustos_rt::shm_grant(region_id, endpoint_id);
        if grant < 0 {
            let _ = rustos_rt::shm_unmap(base as u64, region_len);
        ...
        if rustos_rt::port_bind(notify, NET_CHANNEL_NOTIFY_LEN, NOTIFY_CAPACITY) != 0 {
            let _ = rustos_rt::shm_unmap(base as u64, region_len);
        ...
        if rustos_rt::waitset_ctl(set, WaitSetOp::Add, WaitSourceKind::Port, notify, token) != 0 {
            let _ = client.detach();
            let _ = rustos_rt::shm_unmap(base as u64, region_len);
        ...
        if let Err(err) = stack.add_interface(
            ...
            let _ =
                rustos_rt::waitset_ctl(set, WaitSetOp::Del, WaitSourceKind::Port, notify, token);
            let _ = client.detach();
            let _ = rustos_rt::shm_unmap(base as u64, region_len);
            return Err(err);
        }
    ...
    fn drain_notify(notify: u64) {
        ...
        while rustos_rt::ipc_recv(notify, &mut frame, &mut sender).is_ok() {}
    }

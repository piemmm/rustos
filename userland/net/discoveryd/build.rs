...
//! bare-metal target (`target_os = "none"`), so the `Run` binary
//! (`src/run.rs`) compiles as a freestanding pure-Rust program there and as an
//! inert host stub everywhere else.
//!
//! Keys only off the OS component of the target, never the instruction set,
//! so `cargo xtask cfg-check` stays clean. It mirrors
//! `userland/system/timed/build.rs`.

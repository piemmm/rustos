...
use super::{carry, level, near, shaded, Hoop, Ring, BANDS, LEVELS, MAX_RINGS, STRIP};
use crate::frame::{Basis, Body, Rotation};
use crate::humanoid::palette;
...
fn a_carried_ring_sits_where_its_joint_puts_it() {
    let hoops = carry(&STRAIGHT, Body::new(1.0, 0.0, 0.0), rest(), None, None).expect("carries");
...
fn a_bound_ring_follows_the_joint_that_carries_it() {
    ...
    let straight = carry(&SPAN, Body::ORIGIN, rest(), None, None).expect("carries");
...
fn more_rings_than_a_part_holds_are_refused() {
    ...
    assert!(carry(&many, Body::ORIGIN, rest(), None, None).is_err());
}

//! Build script: enable the `freestanding` cfg when the crate is built for
//! a bare-metal target (`target_os = "none"`), so the `WinterSun.app` `Run`
//! binary (`src/run.rs`) compiles as a freestanding pure-Rust program there
//! and as an inert host stub everywhere else.
//!
//! Keys only off the OS component of the target (bare-metal vs hosted),

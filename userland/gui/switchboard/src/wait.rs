...
//! given the timeout until the next sample deadline. Two members are
//! permanent for the life of the process — the termination signal and the
//! session's command mailbox — and a third, the open window's own event
//! mailbox, is present only while a window is open, so a closed window's
//! channel is never left idly armed.
...
impl WaitToken {
    ...
    ///
    /// Returns [`None`] for any value that is not one of the three defined
    /// tokens — unreachable from a genuine `waitset_wait` reply given the
    /// members the run loop ever arms, but the run loop still treats it as
    /// a spurious wake rather than indexing on a guess (fail closed).
    #[must_use]
...
/// state: [`WaitToken::Signal`] and [`WaitToken::Command`] are permanent;
/// [`WaitToken::WindowEvent`] is present only while `window_open`.
#[must_use]
pub fn required_members(window_open: bool) -> Vec<WaitToken> {
    let mut members = alloc::vec![WaitToken::Signal, WaitToken::Command];
    if window_open {
...
mod tests {
    ...
    fn an_unknown_token_value_decodes_to_none() {
        ...
        assert_eq!(WaitToken::from_u64(4), None);
        assert_eq!(WaitToken::from_u64(u64::MAX), None);
    }

    #[test]
    fn headless_membership_is_signal_and_command_only() {
        assert_eq!(
            required_members(false),
            alloc::vec![WaitToken::Signal, WaitToken::Command]
        );
    }

...
//! bare-metal target (`target_os = "none"`), so the desktop session `Run`
//! binary (`src/run.rs`) compiles as a freestanding pure-Rust program there
//! and as an inert host stub everywhere else (`plans/DISPLAY.md` D7c).
//!

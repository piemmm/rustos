...
//! (`tairix-drv-fs-fat32`) — the same code that mounts it on the running
...
//! [`UnlockDescriptor`](tairix_drv_fs_arxfs::UnlockDescriptor) — the
...
/// parameters (see
/// [`UnlockDescriptor`](tairix_drv_fs_arxfs::UnlockDescriptor)).
///
...
/// cannot drift.
pub use tairix_drv_fs_arxfs::ROOT_UNLOCK_NAME;

/// Render the `config.txt` the Pi firmware boots TAIRiX with.
///
/// The knobs are the P0 facts of record (`docs/src/platform/aarch64.md`,
/// "Boot protocol"): AArch64 cores, the TAIRiX kernel image, a held UART
...
/// (`init_uart_clock=48000000`, `tairix_arch_aarch64::uart_init`), and the
...
pub fn config_txt(with_armstub: bool, console_baud: u32) -> String {
    let mut text = format!(
        "# Generated by tairix-mkimage; do not hand-edit on the card.\n\

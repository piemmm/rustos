# `tairix-cc` — audited, version-pinned, checksummed C toolchain wrapper
...
`tairix_sys_*` syscall stub runtime (`lib/abi-sys`), and the crt0 startup object
(`lib/crt0`) agree with the Rust side end to end (`plans/CCOMPAT.md` stage CC5).
TAIRiX itself stays Rust-only — this crate does **not** add C to the OS.
...
`tairix_cc::REQUIRED_CLANG_VERSION` / `REQUIRED_LLD_VERSION`. A tool that
...
1. **The explicit override** — `TAIRIX_CC_CLANG` / `TAIRIX_CC_LLD`. Authoritative:
...
`TAIRIX_CC_CLANG_SHA256` / `TAIRIX_CC_LLD_SHA256` to the hex digest; a mismatch
fails closed.

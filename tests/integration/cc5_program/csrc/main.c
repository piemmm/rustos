 ...
 *
 * This is a minimal program written in C — not Rust — that talks to the TAIRiX
 * kernel only through the generated abi-v1 C headers (include/tairix/...) and
 * the curated System runtime / C ABI class (crt0 + the tairix_sys_* syscall
 ...
 *
 * TAIRiX itself stays Rust-only (AGENTS.md §1); this program is *hosted* by
...
#define PROBE_CAP TAIRIX_CAP_DRV_KERNEL
...
static int check_time64(void) {
    ...
    /* Sub-second fields stay canonical (0..TAIRIX_NANOS_PER_SEC). */
    if (pre.nanos >= TAIRIX_NANOS_PER_SEC || post.nanos >= TAIRIX_NANOS_PER_SEC) {
        return 0;
    }
    return 1;
}
...
static int check_ipc_header(void) {
    tairix_ipc_message_header_t header;
    header.magic = TAIRIX_IPC_MESSAGE_HEADER_MAGIC;
    header.version = 1u;
    header.flags = 0u;
    ...
    if (header.payload_len > TAIRIX_IPC_MESSAGE_MAX_PAYLOAD_LEN) {
        return 0;
    }
    return 1;
}
...
static int check_sysinfo_header(void) {
    tairix_sysinfo_request_header_t request;
    request.magic = TAIRIX_SYSINFO_REQUEST_MAGIC;
    request.version = TAIRIX_SYSINFO_VERSION_V1;
    request.flags = 0u;
    ...
    if (request.query != TAIRIX_SYSINFO_QUERY_UPTIME) {
        return 0;
    }
    return 1;
}
...
int main(int argc, char **argv, char **envp) {
    ...
    if (tairix_sys_cap_query(PROBE_CAP) != 1u) {
        return EXIT_CAP;
    }
    ...
    if (tairix_sys_clock_get() != CLOCK_SENTINEL) {
        return EXIT_CLOCK;
    }
    return EXIT_OK;
}

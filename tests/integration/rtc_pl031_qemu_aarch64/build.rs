//! Build script for the aarch64 real-time-clock live-boot QEMU vertical
...
//!    conduit, the `virtio,mmio` transport slots, and — the point of this
//!    vertical — the board's `arm,pl031` clock chip, exactly as on real
//!    firmware. The dump helper lives in the shared harness so no aarch64
...
fn main() {
    ...
    let dtb_bytes = if target == "aarch64-unknown-none" {
        ...
        // One CPU: PID 1, the autoloaded driver process, and the devmgr /
        // timed services all run in the same drain loop on the boot CPU.

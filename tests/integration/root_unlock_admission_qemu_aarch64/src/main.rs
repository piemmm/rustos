...
//! the production aarch64 `tairix-kernel` pipeline on the `virt` board with
...
//! (`tairix-test-session-ceiling-qemu-aarch64`, `plans/CAPABILITY_USE.md`
...
mod kernel {
    ...
    #[panic_handler]
    fn tairix_root_unlock_admission_qemu_aarch64_panic(info: &PanicInfo<'_>) -> ! {

//! wasm32 leg of the client's cross-target rendering vertical.
//!
...
//! This one tests a renderer's arithmetic. A WebAssembly engine is all
//! it needs, and asking for a browser would make the fourth Tier-1 leg
//! runnable in fewer places than the claim it defends applies to.
//!
//! So the module exports one function, `frame_digest`, and the harness
//! (`web/harness.mjs`) instantiates it and compares the result against
//! `tairix_wintersun_app::digest::REFERENCE_DIGEST` — the same constant
//! the host suite and the three QEMU verticals assert.
//!
//! wasm32 is the leg most likely to catch a real divergence: it is the
//! only Tier-1 target with a 32-bit `usize`, so a length or an index
//! that had quietly become part of a pixel would show up here and
//! nowhere else.
//!
...
mod vertical {
    ...
    use tairix_wintersun_app::digest;

    /// Static heap the realm field, the generated chunks, the
    /// synthesised material tiles and the two frame buffers are
    /// allocated from — the same one the bare-metal siblings use, rather
    /// than a second allocator written for this target.
    ///
    ...
    #[panic_handler]
    fn client_frame_wasm32_panic(_: &PanicInfo<'_>) -> ! {
        core::arch::wasm32::unreachable()
    }

    /// The digest of the reference frames, or `0` if they could not be
    /// drawn — which the harness reports as a failure rather than
    /// mistaking for an answer.
    #[no_mangle]
    pub extern "C" fn frame_digest() -> u64 {

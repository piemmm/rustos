...
fn main() {
    ...
    let program_dir = format!("{manifest_dir}/../fp_probe_program");
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str, program_dir: &str) -> Vec<u8> {
    ...
    let target_dir = format!("{out_dir}/fp-probe-target");
    ...
    let status = Command::new(cargo)
        ...
        // Pin the program's round count (the single source of truth).
        ...
        .status()
        .expect("spawn cargo to build the fp-probe fixture program");
    assert!(
        status.success(),
        "building the fp-probe fixture program failed"
    );

    let elf_path = format!("{target_dir}/{RISCV64_TARGET}/debug/tairix-test-fp-probe");
    ...
    .expect("convert the fp-probe fixture program ELF into an rxe image")
}
...
fn write_program_fixture(path: &std::path::Path, rxe: &[u8]) {
    ...
    out.push_str("/// The converted `rxe` image of the fp-probe fixture program.\n");

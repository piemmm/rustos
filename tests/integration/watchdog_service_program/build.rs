...
//! bare-metal target (`target_os = "none"`), so the watchdog fixture's `Run`
//! binary (`src/run.rs`) compiles as a freestanding pure-Rust program there
//! and as an inert host stub everywhere else.
//!
//! Self-contained and keyed only off the OS component of the target
//! (bare-metal vs hosted), never the instruction set, so `cargo xtask
//! cfg-check` stays clean. It mirrors `tests/integration/stalltrace_program`.

//! Build-time fixture generator for the SPAWN stage `SP2c` aarch64 EL0↔EL0
//! timeshare vertical.
//!
...
//!    aarch64 target (its own `program.ld` roots `tairix-rt`'s `_start`), into a
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str, program_dir: &str) -> Vec<u8> {
    ...
    let elf_path = format!("{target_dir}/{AARCH64_TARGET}/debug/tairix-test-el0-yielder");

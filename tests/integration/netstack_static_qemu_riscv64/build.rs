//! Build script for the riscv64 static-addressing (`match.node`) netstack
//! live-boot QEMU vertical (`plans/NETWORK.md` N9b-3-2-β-2-ii-b).
//!
...
//! `netstack_autoload_qemu_riscv64` boot vertical does (no duplication).
//! QEMU's OpenSBI firmware hands the boot hart a real device-tree pointer in
//! `a1`, so no embedded DTB fixture is needed here — the planted virtio-blk
//! disk and the attached `virtio-net-device` populate the board's
//! `virtio,mmio` transport slots, which the bootstrap-floor enumeration
//! probes.
//!

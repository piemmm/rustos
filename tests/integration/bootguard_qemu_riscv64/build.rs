//! Build script: hand the riscv64 linker script to `rustc` *only* on the
//! freestanding `riscv64gc-unknown-none-elf` target. On host builds we do nothing so the
//! crate still compiles for `cargo check` / IDE indexing.
//!
//! the charter forbids duplicating the linker script per test, so this
//! crate points at the same file every other riscv64 vertical uses — which

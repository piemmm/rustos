//! QEMU integration test: the riscv64 boot stack's poison guard.
//!
...
mod kernel {
    ...
    use tairix_arch_riscv64::{handle_panic_via_serial, qemu_exit};
    ...
    #[panic_handler]
    fn bootguard_qemu_riscv64_panic(info: &PanicInfo<'_>) -> ! {
    ...
    pub extern "C" fn kernel_main(_hartid: u64, _dtb: u64) -> ! {
        ...
        // SAFETY: the linker script reserves `[low, bottom)` as this hart's
        ...
        match unsafe { check(bt.boot_stack_guard(), low, bottom, sp) } {
            Ok(()) => {
                let _ = writeln!(w, "[bootguard] riscv64 boot-stack guard verified");
                qemu_exit::exit_success()
            }
            Err(defect) => {
                let _ = writeln!(w, "[bootguard] riscv64 FAILED: {}", defect.describe());

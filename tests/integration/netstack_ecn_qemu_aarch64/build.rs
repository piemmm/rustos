//! Build script for the aarch64 ECN (tcpecho + `net.tcp.ecn on`) QEMU
//! vertical (`plans/NETWORK.md` N13).
//!
//! Two jobs on the freestanding `aarch64-unknown-none` target, identical to
//! the stream-socket / memory-stability / root-unlock-admission verticals:
//!
...
//!    dependency's `rustc-link-arg` does not propagate to this bin, so there
//!    is no conflict and this vertical links the `virt` script it runs under.)
...
//!    conduit, and the `virtio,mmio` transport slots exactly as on real
//!    firmware. At run time QEMU populates one slot with the planted
//!    encrypted-root disk and another with the attached virtio-net device,
//!    which the bootstrap-floor enumeration + the autoload path bind. The dump
//!    helper lives in the shared harness so no aarch64 build script re-rolls
//!    it.
//!

...
use super::fwcfg::{FwCfg, KEY_FILE_DIR, KEY_SIGNATURE};
...
/// DRM `XRGB8888` fourcc (`'X','R','2','4'`). In memory the byte order is
/// B, G, R, X, which matches [`DisplayFormat::Bgra8888`] for the purpose
/// of the byte-faithful scan-out copy the driver performs.
const DRM_FORMAT_XRGB8888: u32 = 0x3432_5258;

/// `fw_cfg` file item name QEMU's `ramfb` device registers.
const RAMFB_ITEM: &str = "etc/ramfb";
...
/// Upper bound on directory entries we will scan (keeps the read buffer
/// bounded; a `virt`-board directory is far smaller).
...
/// Confirm the `fw_cfg` device answers with its `QEMU` signature (a
/// round-trip sanity check on the DMA read path before we drive a write).
fn check_signature(fw: &FwCfg) {
    let mut sig = [0u8; 4];
    if fw.read_item(KEY_SIGNATURE, &mut sig).is_err() {
        fail("fw_cfg: signature read failed");
    }
    if &sig != b"QEMU" {
        fail("fw_cfg: signature mismatch");
    }
}

/// Find the `etc/ramfb` item's selector key in the `fw_cfg` file
/// directory. Never returns on failure.
fn ramfb_selector(fw: &FwCfg) -> u16 {
    let mut count_buf = [0u8; 4];
    if fw.read_item(KEY_FILE_DIR, &mut count_buf).is_err() {
        fail("fw_cfg: file-dir count read failed");
    }
    let count = u32::from_be_bytes(count_buf) as usize;
    if count == 0 || count > MAX_DIR_ENTRIES {
        fail("fw_cfg: implausible file-dir entry count");
    }
    let mut entries = vec![0u8; count * DIR_ENTRY_LEN];
    if fw.read_more(&mut entries).is_err() {
        fail("fw_cfg: file-dir entries read failed");
    }
    for entry in entries.chunks_exact(DIR_ENTRY_LEN) {
        ...
        if &name_bytes[..end] == RAMFB_ITEM.as_bytes() {
            return u16::from_be_bytes([entry[4], entry[5]]);
        }
    }
    fail("fw_cfg: etc/ramfb item not found")
}

/// Program `ramfb` to scan out from [`FRAMEBUFFER`] with the vertical's
/// geometry by writing a big-endian `RAMFBCfg` to its `fw_cfg` item.
fn program_ramfb(fw: &FwCfg, selector: u16) {
    let mut cfg = [0u8; 28];
    cfg[0..8].copy_from_slice(&framebuffer_phys().to_be_bytes());
    cfg[8..12].copy_from_slice(&DRM_FORMAT_XRGB8888.to_be_bytes());
    ...
    if fw.write_item(selector, &cfg).is_err() {
        fail("fw_cfg: ramfb config write failed");
    }
}
...
pub fn run() {
    ...
    let Ok(fw) = FwCfg::from_dtb(&dtb) else {
        fail("no fw_cfg device in DTB");
    };
    check_signature(&fw);
    let selector = ramfb_selector(&fw);
    program_ramfb(&fw, selector);

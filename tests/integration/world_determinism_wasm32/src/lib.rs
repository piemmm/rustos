//! wasm32 leg of the world generator's cross-architecture determinism
//! vertical.
//!
...
//!
//! So the module exports one function, `world_digest`, and the harness
...
mod vertical {
    ...
    use tairix_wintersun_world::digest;

    /// Static heap the generator's working set is allocated from — the
    /// same one the bare-metal siblings use, rather than a second
    /// allocator written for this target.
    ///
    ...
    #[panic_handler]
    fn world_determinism_wasm32_panic(_: &PanicInfo<'_>) -> ! {
        core::arch::wasm32::unreachable()
    }

    /// The digest of the reference realm, or `0` if it could not be
    /// solved — which the harness reports as a failure rather than
    /// mistaking for an answer.
    #[no_mangle]
    pub extern "C" fn world_digest() -> u64 {
        digest::world(digest::reference_params()).unwrap_or(0)
    }

//! Build script for the riscv64 real-time-clock
//! live-boot QEMU vertical (`plans/TIMESYNC.md` TS-3).
//!
...
//! script the architecture port owns, exactly as every sibling
//! riscv64 boot vertical does (no duplication).
//! QEMU's OpenSBI firmware hands the boot hart a real device-tree pointer in
//! `a1`, so no embedded DTB fixture is needed here; that tree is also where
//! the board's `google,goldfish-rtc` node comes from, which is what this
//! vertical exercises.
//!

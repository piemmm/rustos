...
//! (`rustos_kernel_core::system_identity_table`) into the spawn-as-user
...
static DEVMGR_UID_ARG: [u8; DEC_MAX] = dec_bytes(rustos_users::DEVMGR_UID.0);
...
const SVC_MANIFEST_LEN: usize = rustos_users::DEVMGR_CEILING.len() + SIBLING_CAPS.len();
...
const fn svc_manifest() -> [CapabilityId; SVC_MANIFEST_LEN] {
    ...
    let mut i = 0;
    while i < rustos_users::DEVMGR_CEILING.len() {
        out[i] = rustos_users::DEVMGR_CEILING[i];
        i += 1;
    }
    let mut j = 0;
...
pub extern "C" fn kernel_main(_dtb: u64) -> ! {
    ...
    let Ok(table) = rustos_kernel_core::system_identity_table(&SERIAL_SINK) else {
    ...
    // `rustos-rt` heap) and the one-page-per-fault backing the

//! Build-time fixture generator for the USERS U4 aarch64 service-ceiling
//! vertical.
//!
...
//!    (`tests/integration/service_ceiling_program`) **once** — the two
//!    roles are selected at runtime from the registry argument vector —
...
fn main() {
    ...
    let program_dir = format!("{manifest_dir}/../service_ceiling_program");
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str, program_dir: &str) -> Vec<u8> {
    ...
    let target_dir = format!("{out_dir}/service-ceiling-target");
    ...
    command
        ...
        // target-scoped flags win and apply only to the aarch64 program
        // crates.
    ...
    let status = command
        .status()
        .expect("spawn cargo to build the service-ceiling fixture program");
    assert!(
        status.success(),
        "building the service-ceiling fixture program failed"
    );

    let elf_path = format!("{target_dir}/{AARCH64_TARGET}/debug/rustos-test-service-ceiling");
    ...
    .unwrap_or_else(|_| panic!("convert the service-ceiling fixture program ELF into an rxe image"))
}
...
fn write_program_fixture(path: &std::path::Path, rxe: &[u8]) {
    ...
    let _ = writeln!(
        out,
        "/// The converted `rxe` image of the two-role fixture program."
    );

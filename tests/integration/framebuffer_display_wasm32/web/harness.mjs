// Browser-headless harness for the wasm32 framebuffer-display vertical
// (`plans/WIRING.md` — the wasm32 `display`-row parity gap).
//
// This is the wasm32 analogue of the bare-metal `tools/qemu` runner and
// is a sibling of the boot vertical's harness: it boots the compiled
// kernel module in a real (headless) browser and decides PASS/FAIL from
// what the kernel prints. `cargo xtask test --wasm` builds the `.wasm`
// and launches this script.
//
// It asserts the deliverables:
//   * BOOT_OK    — the wasm32 Arch HAL booted and entered `kernel_main`.
//   * DISPLAY_OK — the signed framebuffer display `.rxe` was loaded,
//                  driven through load -> use -> unload -> reload, and
//                  each presented frame reached both linear memory (an
//                  independent capability-gated window read-back) and a
//                  genuine display surface (the canvas round-trip).
//
...
function parseArgs(argv) {
  const args = { timeoutSecs: 30, chrome: null, wasm: null };
...
async function run() {
  ...
  try {
    ...
    const check = () => {
      if (lines.includes("BOOT_OK") && lines.includes("DISPLAY_OK")) {
        pass = true;
        resolveDone();
      }
    ...
    console.log(
      `[wasm display harness] BOOT_OK=${lines.includes("BOOT_OK")} ` +
        `DISPLAY_OK=${lines.includes("DISPLAY_OK")}`,
    );
    if (!pass) {
      console.log(`[wasm display harness] FAIL (${reason})`);
      console.log(`[wasm display harness] captured lines: ${JSON.stringify(lines)}`);
    }
  } finally {
    await browser.close();
    server.close();
  ...
  if (pass) {
    console.log("[wasm display harness] PASS");
    process.exit(0);
  }
  process.exit(1);
}
...
run().catch((err) => {
  console.error(`[wasm display harness] error: ${err && err.stack ? err.stack : err}`);
  process.exit(1);
});

...
//!    signal_program`) **twice** — once as the `child` role and once as the
...
//!    so [`rustos_abi::rxe::LoadImage::parse`] accepts it; emit the two blobs
...
/// Emit `CHILD_RXE`, `PARENT_RXE`, and `USER_BIAS` as a Rust source the test
/// includes.

//! Build script for the riscv64 driver-loading-by-discovery autoload QEMU
//! vertical (`plans/NETWORK.md` N4e-riscv64, first stage).
//!
...
//! `spawn_init_qemu_riscv64` boot vertical does. Unlike the aarch64 `-kernel`
//! path (which passes `x0 = 0` and so embeds a DTB fixture), the riscv64
//! OpenSBI firmware hands the boot hart a real device-tree pointer in `a1`,
//! so no embedded DTB is needed here — the planted virtio-blk disk and the
//! attached `virtio-keyboard-device` populate the board's `virtio,mmio`

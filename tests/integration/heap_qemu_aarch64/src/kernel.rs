...
impl MemMap for AnonProducer {
    fn map(&self, len: usize, flags: MapFlags, addr_hint: u64) -> Result<u64, Errno> {
        ...
        // The `rustos-rt` heap always maps with FIXED placement at the next
...
///
/// `mem_map`/`mem_unmap` (issued by the heap allocator inside `rustos-rt`) are
...
/// its allocations cleanly and exits), so any fault is a failure (`AGENTS.md`
/// §2.9 — never returns).

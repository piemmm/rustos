...
//!    aarch64 target (its own `program.ld` roots crt0's `_start`), into a
...
/// instead of colliding with an identity gigapage block. The kernel passes
/// the same bias to `build_process_image`, and `elf_to_rxe` relocates the
/// image for it, so the in-memory pointers match where it is mapped.
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str, program_dir: &str) -> Vec<u8> {
    ...
    // (`AGENTS.md` §19.2). Scope the PIE link flags to the aarch64 target so

//! The freestanding aarch64 test kernel: build an EL0 image for the fixture
...
/// startup-vector block buffer. The block for `["prog", "42"]` is a few
/// hundred bytes; 2 MiB is generous headroom. It lives in `.bss` (zeroed by
/// the boot trampoline); the aarch64 `virt` linker script has no dedicated
/// `.heap` section, so unlike the riscv64 sibling the heap is a plain `.bss`
/// static — `Vec` zeroes each allocation, so an unzeroed heap would be
/// harmless regardless.
...
#[panic_handler]
fn spawn_program_qemu_aarch64_panic(info: &PanicInfo<'_>) -> ! {
...
pub extern "C" fn kernel_main(_dtb: u64) -> ! {
    ...
    note(
        TEST_START,
        "aarch64 spawn round-trip: building the EL0 image",
    );

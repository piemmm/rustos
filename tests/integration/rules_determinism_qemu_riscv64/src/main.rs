...
mod kernel {
    ...
    const TEST_START: EventId = EventId(4523);
    const TEST_PASS: EventId = EventId(4524);
    const TEST_FAIL: EventId = EventId(4525);
    ...
    #[panic_handler]
    fn rules_determinism_qemu_riscv64_panic(info: &PanicInfo<'_>) -> ! {
    ...
    pub extern "C" fn kernel_main(_hartid: u64, _dtb: u64) -> ! {
        ...
        if produced == REFERENCE_DIGEST {
            ...
            note(
                TEST_PASS,
                "rules determinism: riscv64 agrees with the reference",
            );

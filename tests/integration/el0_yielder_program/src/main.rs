...
//! calls `tairix_rt::yield_now` `TAIRIX_EL0_YIELDS` times — every `yield` is a
...
mod program {
    ...
    /// pin a count via the `TAIRIX_EL0_YIELDS` environment variable. Large
    ...
    /// The yield count, read from the `TAIRIX_EL0_YIELDS` environment variable
    ...
    const fn yield_count() -> u32 {
        match option_env!("TAIRIX_EL0_YIELDS") {

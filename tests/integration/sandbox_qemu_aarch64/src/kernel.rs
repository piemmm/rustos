...
//! `LiveMemMap` producer (the `rustos-rt` heap's `mem_map` pair), a real
...
/// replacement, probe — image, stack, page tables, and the `rustos-rt`
...
/// wedging the machine.
extern "C" fn sandbox_user_fault(far: u64, write: bool) -> bool {
...
pub extern "C" fn kernel_main(_dtb: u64) -> ! {
    ...
    // `rustos-rt` heap (the parent's and each worker's) draws on.

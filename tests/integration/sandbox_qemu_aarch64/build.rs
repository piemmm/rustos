//! Build-time fixture generator for the S8b aarch64 parser-sandbox
//! vertical.
//!
...
//!    (`tests/integration/sandbox_program`) **once** — the four roles are
//!    selected at runtime from the registry path + the seam's worker
//!    marker — position-independent for the freestanding aarch64 target.
...
fn main() {
    ...
    let program_dir = format!("{manifest_dir}/../sandbox_program");
    ...
    // The fixture program compiles the seam in, so a seam change must
    // rebuild it.
    println!("cargo:rerun-if-changed={manifest_dir}/../../../lib/sandbox/src");
...
/// Compile the EL0 fixture program PIE for the freestanding aarch64 target
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str) -> Vec<u8> {
    ...
    let target_dir = format!("{out_dir}/sandbox-target");

    // Cargo fingerprints the RUSTFLAGS *string* (which names the linker
    // script by path) but not the script's *content*, so a `program.ld`
    // edit would not by itself trigger a relink and the converter could
    // read a stale ELF. Wiping the private target directory here forces a
    // clean rebuild against the current script without churning ordinary
    // incremental builds.
    ...
    command
        ...
        // the target-scoped flags win and apply only to the aarch64
        // program crates.
    ...
    let status = command
        .status()
        .expect("spawn cargo to build the sandbox fixture program");
    assert!(
        status.success(),
        "building the sandbox fixture program failed"
    );

    let elf_path = format!("{target_dir}/{AARCH64_TARGET}/debug/tairix-test-sandbox");
    ...
    .unwrap_or_else(|_| panic!("convert the sandbox fixture program ELF into an rxe image"))
}

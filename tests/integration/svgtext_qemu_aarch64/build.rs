//! Build script for the aarch64 SVG-text QEMU vertical
//! (`plans/SVG.md` S23/S24).
//!
//! Two jobs on the freestanding `aarch64-unknown-none` target, identical
//! to the root-unlock-admission / spawn-session verticals:
//!
//! 1. Hand the aarch64 `virt` linker script to `rustc` — the single
//!    per-board linker script the architecture port owns. (`tairix-kernel`'s
//!    own build script links the Raspberry Pi 4 script for its *binary*; a
//!    dependency's `rustc-link-arg` does not propagate to this bin, so
//!    there is no conflict and this vertical links the `virt` script it
//!    actually runs under.)
...
//!    real firmware. With the planted encrypted-root disk attached at run
//!    time QEMU populates one slot's live `DeviceID` register, which the
...
fn main() {
    ...
    let dtb_bytes = if target == "aarch64-unknown-none" {
        ...
        // processes, the font service, and the fixture's own sandbox
        // workers all run on the boot CPU.

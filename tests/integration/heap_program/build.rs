//! Build script: enable the `freestanding` cfg when the heap fixture program
//! is built for a bare-metal target (`target_os = "none"`), so `src/main.rs`
//! compiles as a freestanding pure-Rust program there and as an inert host stub
//! everywhere else (mirrors `mem_map_program/build.rs`).
//!

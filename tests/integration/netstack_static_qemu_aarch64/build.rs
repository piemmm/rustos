//! Build script for the aarch64 static-addressing (`match.node`) netstack
...
//! the two-process autoload vertical it is modelled on:
//!
...
//!    firmware. With the planted virtio-blk-mmio disk *and* the attached
//!    `virtio-net-device` at run time, QEMU populates two slots' live
//!    `DeviceID` registers — one virtio-block (the encrypted root) and one
//!    virtio-net (at the stable register base the planted `network.conf`'s
//!    `match.node` names) — which the bootstrap-floor enumeration probes. The
...
fn main() {
    ...
    let dtb_bytes = if target == "aarch64-unknown-none" {
        ...
        // One CPU: PID 1, the unlock kthread, the autoloaded driver processes,
        // the netstack service, and the devmgr service all run in the same
        // cooperative drain loop on the boot CPU.

//! Build-time fixture generator for the `SP11c` aarch64 demand-grown stack
//! vertical.
//!
...
fn main() {
    ...
    let program_dir = format!("{manifest_dir}/../threads_program");
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str, program_dir: &str) -> Vec<u8> {
    ...
    let target_dir = format!("{out_dir}/threads-target");
    ...
    let status = command
        .status()
        .expect("spawn cargo to build the threads fixture program");
    assert!(
        status.success(),
        "building the threads fixture program failed"
    );

    let elf_path = format!("{target_dir}/{AARCH64_TARGET}/debug/tairix-test-threads");
    ...
    .unwrap_or_else(|_| panic!("convert the threads fixture program ELF into an rxe image"))
}

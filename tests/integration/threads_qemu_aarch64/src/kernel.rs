//! The freestanding aarch64 test kernel: prove lightweight threads end to
...
const TEST_START: EventId = EventId(4490);
const TEST_SPAWNED: EventId = EventId(4491);
const TEST_PASS: EventId = EventId(4492);
...
#[panic_handler]
fn threads_qemu_aarch64_panic(info: &PanicInfo<'_>) -> ! {
...
pub extern "C" fn kernel_main(_dtb: u64) -> ! {
    note(TEST_START, "aarch64 threads test: starting");
    ...
    note(
        TEST_SPAWNED,
        "aarch64 threads test: parent spawned through the production seam",
    );
        ...
        match wait_producer.poll(ProcessId(0), parent_pid, WaitFlags::empty()) {
            Ok(reaped) => match reaped.status {
                WaitStatus::Exited(0) => {
                    note(
                        TEST_PASS,
                        "aarch64 threads test: parent verified every thread role and exited 0",
                    );

//! Build-time fixture generator for the CCOMPAT stage CC5 aarch64 C-program
//! round-trip.
//!
...
//!    position-independent `staticlib` for the freestanding aarch64 target —
...
/// aarch64 round-trip: 64 GiB, far above the kernel's 2 GiB identity map and
/// within the 39-bit TTBR0 region, so the program's pages land on freshly
/// walked stage-1 tables.
...
/// Build the aarch64 PIE C image and convert it to an `rxe` blob.
fn build_c_program(
    manifest_dir: &str,
    out_dir: &str,
    c_source: &Path,
...
/// `staticlib` for the freestanding aarch64 target, returning its `.a` path.
fn build_runtime_shim(manifest_dir: &str, out_dir: &str) -> PathBuf {
    ...
    PathBuf::from(format!(
        "{target_dir}/{AARCH64_TARGET}/release/librustos_test_cc5_program.a"
    ))
}

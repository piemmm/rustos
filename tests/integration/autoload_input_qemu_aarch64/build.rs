//! Build script for the aarch64 driver-loading-by-discovery autoload QEMU
//! vertical (`plans/PI.md` P10 5d-2-ii(b-2-iii)).
//!
//! Two jobs on the freestanding `aarch64-unknown-none` target, identical to the
...
//!    it discovers the console, GIC, `/memory` window, timer rate, PSCI conduit,
//!    and the `virtio,mmio` transport slots exactly as on real firmware. With
//!    the planted virtio-blk-mmio disk *and* the attached `virtio-keyboard`
...
fn main() {
    ...
    let dtb_bytes = if target == "aarch64-unknown-none" {
        ...
        // One CPU: PID 1, the unlock kthread, the autoloaded driver process,
        // and the console login all run in the same cooperative drain loop on
        // the boot CPU.

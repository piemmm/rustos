...
//! ## How it asserts it
//!
//! Using the Stage-3 Sv39 primitives (`rustos_arch_riscv64::paging`) it:
//!
...
//!    *only* the aliased stub and its stack — the §4 isolation contract.
...
//! U-mode at all is a distinct closed failure (`AGENTS.md` §5.4.5).
//!
...
compile_error!(
    "rustos-test-abi-sys-syscall-qemu-riscv64: the `test-hooks` Cargo feature is a \
...
mod kernel {
    ...
    #[panic_handler]
    fn rustos_abi_sys_syscall_qemu_riscv64_panic(info: &PanicInfo<'_>) -> ! {
    ...
    /// what `ros_sys_cap_query(EXPECTED_CAP)` should have placed in the
...
#[cfg(all(itest_riscv64, not(feature = "test-hooks")))]
#[panic_handler]
fn rustos_abi_sys_syscall_qemu_riscv64_panic_stub(_info: &core::panic::PanicInfo<'_>) -> ! {
    loop {

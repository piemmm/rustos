//! Build script for the aarch64 spawn-the-session QEMU vertical
//! (`plans/SPAWN.md` `SP3b`).
//!
//! Two jobs on the freestanding `aarch64-unknown-none` target, identical
//! to the P6c-3 spawn-PID-1 vertical:
//!
...
fn main() {
    ...
    let dtb_bytes = if target == "aarch64-unknown-none" {
        ...
        // One CPU: PID 1 is spawned on the boot CPU and the session runs in
        // the same cooperative drain loop.

...
//!    `TAIRIX_WAIT_ROLE`) so this script is the single source of truth for both. Each linked PIE ELF is converted to an `rxe` blob with
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str, role: &str) -> Vec<u8> {
    ...
    let elf_path = format!("{target_dir}/{RISCV64_TARGET}/debug/tairix-test-wait");

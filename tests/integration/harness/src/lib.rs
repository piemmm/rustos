...
/// directory, or the fixture cannot be written: a build script cannot go on
/// without them.
pub fn aarch64_virt_guest_build(cpus: u32) {
    ...
    let dtb = if std::env::var("TARGET").is_ok_and(|target| target == "aarch64-unknown-none") {
        let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR");
        let linker_script = format!(
        ...
        println!("cargo:rerun-if-changed={linker_script}");
        println!("cargo:rustc-link-arg=-T{linker_script}");
        dump_aarch64_virt_dtb(&out_dir, cpus)
    } else {
        Vec::new()
    };
    ...
    out.push_str("\n];\n");
    std::fs::write(
        std::path::PathBuf::from(&out_dir).join("dtb_fixture.rs"),
        out,
    )
    .expect("write dtb_fixture.rs");
}

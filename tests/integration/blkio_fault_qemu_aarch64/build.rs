...
fn main() {
    ...
    let program_dir = format!("{manifest_dir}/../blkio_fault_program");
...
fn build_and_convert_program(manifest_dir: &str, out_dir: &str, program_dir: &str) -> Vec<u8> {
    ...
    let target_dir = format!("{out_dir}/blkio-fault-target");
    ...
    let status = command
        .status()
        .expect("spawn cargo to build the block-transport fault fixture program");
    assert!(
        status.success(),
        "building the block-transport fault fixture program failed"
    );

    let elf_path = format!("{target_dir}/{AARCH64_TARGET}/debug/tairix-test-blkio-fault");
    ...
    .unwrap_or_else(|_| {
        panic!("convert the block-transport fault fixture program ELF into an rxe image")
    })
}

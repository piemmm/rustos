//! The freestanding aarch64 test kernel: prove the demand-grown user stack
//! end to end (`plans/SPAWN.md` `SP11c`;
...
//! same slot, wires the production `LiveMemMap` producer (backing both
...
use rustos_kernel::spawn_layout::{USER_STACK_COMMIT_PAGES, USER_STACK_RESERVE_PAGES};
...
const TEST_START: EventId = EventId(4360);
const TEST_SPAWNED: EventId = EventId(4361);
...
#[panic_handler]
fn stack_grow_qemu_aarch64_panic(info: &PanicInfo<'_>) -> ! {
...
/// fault-exit path.
extern "C" fn stack_grow_user_fault(far: u64, write: bool) -> bool {
...
pub extern "C" fn kernel_main(_dtb: u64) -> ! {
    note(TEST_START, "aarch64 stack-grow test: starting");
    ...
    note(
        TEST_SPAWNED,
        "aarch64 stack-grow test: parent spawned through the production seam",
    );
        ...
        match wait_producer.poll(SecTaskId(0), parent_pid, WaitFlags::empty()) {
            Ok(reaped) => match reaped.status {
                WaitStatus::Exited(0) => {
                    note(
                        TEST_PASS,
                        "aarch64 stack-grow test: parent verified all three children and exited 0",
                    );

//! Build script: enable the `freestanding` cfg when the block-transport
//! fault fixture program is built for a bare-metal target
//! (`target_os = "none"`), so `src/main.rs` compiles as a freestanding
//! pure-Rust program there and as an inert host stub everywhere else
//! (mirrors `tests/integration/service_ceiling_program`).
//!

...
//! [`tairix_kernel_mem::LiveSpace::map_device_window`] (`plans/PI.md`
//! 5d-0-ii (b′)-2).
...
/// [`LiveSpace`]'s [`tairix_kernel_mem::MmioWindowMap`] hands the `mmio_map`
...
/// retained [`LiveSpace`]'s [`tairix_kernel_mem::AnonWindowMap`] places each
...
/// [`LiveSpace`]'s [`tairix_kernel_mem::DmaWindowMap`] carves each
...
/// Base of the program's cross-process shared-memory virtual region: the
/// retained [`LiveSpace`]'s shared-window allocator maps each granted
/// `shm_map` region out of this range. 4 GiB above [`USER_BIAS`] - above the
/// DMA window and clear of the image/stack - mirroring the production
/// aarch64 spawn layout (`spawn_layout::SHARED_WINDOW_OFFSET`).
const SHARED_WINDOW_BASE: u64 = USER_BIAS + 0x1_0000_0000;
...
const FAIL_LIVE_BUILD: NonZeroU16 = fail_point!(10);
const FAIL_DRAINED: NonZeroU16 = fail_point!(11);
const FAIL_FAULT_INSTALL: NonZeroU16 = fail_point!(12);
const FAIL_FAULT: NonZeroU16 = fail_point!(13);
...
/// error (the encoding `tairix_rt::mmio_map` decodes).
...
/// The custody the program's DMA carves bind: the production quarantine,
/// returning frames to the same pool the retained space draws them from.
static DMA_CUSTODY: Once<DmaQuarantine> = Once::new();

/// The node the stand-in DMA grant belongs to, and the admission generation of
/// the one driver-shaped program this vertical runs.
...
pub extern "C" fn kernel_main(_dtb: u64) -> ! {
    ...
    let Ok(image) = tairix_abi::rxe::LoadImage::parse(PROGRAM_RXE, &SYSCALL_TABLE_HASH) else {
    ...
    // SAFETY: as for `identity` above; the quarantine scrubs only frames it
    // was surrendered, all of which lie in this identity-mapped window.
    let Some(custody_map) = (unsafe { DirectPhysMap::identity((IDENTITY_GIB as u64) << 30) })
    else {
        qemu_exit::exit_failure(FAIL_LIVE_BUILD);
    };

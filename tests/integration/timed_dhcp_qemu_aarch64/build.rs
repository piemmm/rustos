//! Build script for the aarch64 DHCP-supplied-time-server live-boot QEMU
//! vertical (`plans/TIMESYNC.md` TS-7).
//!
//! Two jobs on the freestanding `aarch64-unknown-none` target, identical to
//! the DHCP-addressing vertical it is modelled on:
//!
...
fn main() {
    ...
    let dtb_bytes = if target == "aarch64-unknown-none" {
        ...
        // and the netstack / devmgr / timed services all run in the same

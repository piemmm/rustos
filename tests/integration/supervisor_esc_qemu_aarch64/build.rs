//! Build script for the aarch64 pre-boot-Supervisor ESC QEMU vertical
//! (`plans/NEW-SUPERVISOR.md` §7).
//!
//! Two jobs on the freestanding `aarch64-unknown-none` target, identical
//! to the root-unlock-admission vertical this mirrors:
//!
//! 1. Hand the aarch64 `virt` linker script to `rustc` — the single
//!    per-board linker script the architecture port owns. (`tairix-kernel`'s own build script links the Raspberry Pi 4
//!    script for its *binary*; a dependency's `rustc-link-arg` does not
//!    propagate to this bin, so there is no conflict and this vertical
//!    links the `virt` script it actually runs under.)
...
//!    PSCI conduit, and the `virtio,mmio` transport slots exactly as on
//!    real firmware. The `virt` tree always describes its virtio-MMIO
//!    slots; with the planted virtio-blk-mmio disk attached at run time
//!    QEMU populates one slot's live `DeviceID` register, which the
//!    bootstrap-floor enumeration reads to bind the root. The dump helper
//!    lives in the shared harness so no aarch64 build script re-rolls it.
//!
...
fn main() {
    ...
    let dtb_bytes = if target == "aarch64-unknown-none" {
        ...
        // One CPU: PID 1, the unlock kthread, and the console login all run
        // in the same cooperative drain loop on the boot CPU.

...
inside RustOS — and is consumed only as a `[dev-dependencies]` entry of the
harness crates, so it never enters a RustOS build graph. The seed is a
...
```
[fuzzseed] <test>: PRNG seed = 12345 (0x0000000000003039); replay with RUSTOS_FUZZ_SEED=12345
```
...
```
RUSTOS_FUZZ_SEED=12345 cargo test -p <crate> --test <harness>
